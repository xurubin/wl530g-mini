...
// check duplicated
function validate_duplicate_noalert_i(o, v, l, off, i)
{
...
function markGroup(o, s, c, b) {
    ...
    if (b == " Add ")
    {
        ...
        else if (s=='GWStatic')
        {
            ...
            else if (!validate_gwstaticlist(document.form.GWStatic_s, document.form.sr_ipaddr_x_0.value,document.form.sr_netmask_x_0.value
		, document.form.sr_gateway_x_0.value, document.form.sr_if_x_0.value)) return false;
        ...
        {        
            ...
            else
            {
            	...
            	{ 
                	...
                	if (!validate_portrange(document.form.vts_port_x_0, "") || !validate_ipaddr(document.form.vts_ipaddr_x_0, "") ||
            		!validate_range_sp(document.form.vts_lport_x_0, 1, 65535)) return false;
                	else if (document.form.vts_port_x_0.value=="") bFlag=1;    
                	
                	for (i=0; i<document.form.VSList_s.options.length; i++)    
			{        	
                	  if (!validate_duplicate_noalert_i(document.form.VSList_s, document.form.vts_port_x_0.value, 5, 0, i)&&
            	     	 !validate_duplicate_noalert_i(document.form.VSList_s, document.form.vts_ipaddr_x_0.value, 15, 12, i)&&
            	     	 !validate_duplicate_noalert_i(document.form.VSList_s, document.form.vts_proto_x_0.value, 5, 35, i)) 
            	     	 	{
        ...
        else if (s=='LWFilterList')
        {
            ...
            else
            {
            	...
            	!validate_portrange(document.form.filter_lw_dstport_x_0, "")) return false;
		else if (!validate_lanwanlist(document.form.LWFilterList_s, document.form.filter_lw_srcip_x_0.value, 
			document.form.filter_lw_dstport_x_0.value, document.form.filter_lw_proto_x_0.value)) return false;
            	else if (document.form.filter_lw_srcip_x_0.value=="" &&                	
                	document.form.filter_lw_dstport_x_0.value=="") bFlag=1;                                 
            }    		
        }                 
...
function validate_lanwanlist(o,v1, v2, v3)
{
	var i;
	for(i=0; i < o.options.length; i++)
	{
		if (( entry_cmp(o.options[i].text.substring(0).toLowerCase(), v1.toLowerCase(), 15) ==0 ) &&
			(entry_cmp(o.options[i].text.substring(16).toLowerCase(), v2.toLowerCase(), 10) == 0) &&
			(entry_cmp(o.options[i].text.substring(28).toLowerCase(), v3.toLowerCase(), 5) ==0))	
		{
			i =i+1;
			alert('This entry has been in table!!!');
			return false;
...
function validate_gwstaticlist(o, v1, v2, v3, v4)
{
	var i;
	for(i=0; i < o.options.length; i ++)
	{
		if ((entry_cmp(o.options[i].text.substring(0).toLowerCase(), v1.toLowerCase(), 15)==0) &&
			(entry_cmp(o.options[i].text.substring(17).toLowerCase(),v2.toLowerCase(), 15) ==0) &&
			(entry_cmp(o.options[i].text.substring(34).toLowerCase(),v3.toLowerCase(), 15)==0) &&
			(entry_cmp(o.options[i].text.substring(56).toLowerCase(),v4.toLowerCase(), 5) ==0))
		{ 
			i = i+1;
			alert('This entry has been in table!!!');	
			return false;
...
{   	      	   	   	
   if (v == "wl_wpa_psk")
   {
   	if (document.form.wl_auth_mode.value == "psk" || document.form.wl_auth_mode.value == "psk2")
   	{   	   
   	   if (o.value.length < 8)
   	   {	
...
{    
    ...
    else if (document.form.wl_auth_mode.value == "psk" || document.form.wl_auth_mode.value == "psk2")
    {
...
function wl_wep_change()
{
	...
	if (window.top.isModel()=="WL520" || window.top.isModel()=="SnapAP" || window.top.isCard()=="ralink")
	{
		if (mode == "wpa" || mode == "psk" || mode == "psk2"|| mode == "radius") 
		{
			inputCtrl(document.form.wl_crypto,  1);
        		inputCtrl(document.form.wl_wpa_psk,  1);
	...
	else
	{
		...
		if (wep != "0")
			...
			if (mode == "wpa" || mode == "psk" || mode == "psk2")
				inputCtrl(document.form.wl_wpa_gtk_rekey,  1); 	
			else
				inputCtrl(document.form.wl_wpa_gtk_rekey,  0);
		}
...
function wl_auth_mode_reconf()
{
	...
	document.form.wl_auth_mode.options[4].value = null;
	document.form.wl_auth_mode.options[4] = null;
	document.form.wl_auth_mode.options[4].value = null;
	document.form.wl_auth_mode.options[4] = null;	
}
