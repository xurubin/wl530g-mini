...
int set_server (char *word, char *value, int context, void *item)
{
    struct lac *l = (struct lac *) item;
    struct lns *n = (struct lns *) item;
    switch (context & ~CONTEXT_DEFAULT)
    {
    case CONTEXT_LNS:
        if (set_string (word, value, n->server_name, sizeof (n->server_name)))
            return -1;
        break;
    case CONTEXT_LAC:
        if (set_string (word, value, l->server_name, sizeof (l->server_name)))
            return -1;
        break;
    default:
        snprintf (filerr, sizeof (filerr), "'%s' not valid in this context\n",
