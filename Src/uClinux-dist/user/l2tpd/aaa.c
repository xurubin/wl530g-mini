...
#define SIZE_OF_AVP_HEADER sizeof(struct avp_hdr)-2
...
//        sizeof (struct avp_hdr);
	((len - SIZE_OF_AVP_HEADER +1) /16 + 1) * 16 +
	SIZE_OF_AVP_HEADER;
...
//    old_hdr->attr = htons (len - sizeof (struct avp_hdr));
	old_hdr->attr = htons (len - SIZE_OF_AVP_HEADER);
...
//    ptr = ((char *) new_hdr) + sizeof (struct avp_hdr);
	ptr = ((char *) new_hdr) + SIZE_OF_AVP_HEADER;
...
//    ptr = ((char *) old_hdr) + sizeof (struct avp_hdr);
	ptr = ((char *)old_hdr) + SIZE_OF_AVP_HEADER;
...
//    len = ntohs (new_hdr->attr) + sizeof (struct avp_hdr);
	len = ntohs (new_hdr->attr) + SIZE_OF_AVP_HEADER;
    if (len > olen - 2)
    {
        log (LOG_DEBUG,
