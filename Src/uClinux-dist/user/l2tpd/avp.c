...
#define SIZE_OF_AVP_HEADER sizeof(struct avp_hdr)-2 //SJ_Yen
...
//        log (LOG_DEBUG, "raw0=%x, raw1=%x, raw2=%x, raw3=%x, info=%x, ntohs=%x,\n",raw[0],raw[1],raw[2],raw[3],c->msgtype, ntohs(raw[3]));
//        log (LOG_DEBUG, "htons[2]=%x,ntohs[2]=%x \n",htons(2),ntohs(2));
    if (datalen != 8)
    {
        if (DEBUG)
...
int handle_avps (struct buffer *buf, struct tunnel *t, struct call *c)
{
	...
	char avp_buf[128], *tmp; // avp_hed[128];
	struct avp_hdr *avp_h;
	int x=0;
...
//	memcpy((void *)avp_hed, (void *)avp, SIZE_OF_AVP_HEADER);
	
	super_safe_copy ((unsigned char *)avp_h ,(unsigned char *)avp, SIZE_OF_AVP_HEADER);
	
//	log(LOG_WARN,"MSG11111:::avp_hed=%x, avp=%x\n", avp_hed, avp);
//	log(LOG_WARN,"MSG22222:::avp_hed=%x, avp=%x\n", &avp_hed, &avp);

/*	tmp = (unsigned char *)avp_hed;
	for (x=0;x<8;x++){
	log(LOG_WARN,"!!msg:::avp_hed[%d]=%x\n", x, tmp[x] );
	}
*/
//	avp_h = (struct avp_hdr *)avp_hed;
//	tmp = (unsigned char *)avp_h;
//	for (x=0;x<8;x++){
  //      log(LOG_WARN,"!!msg:::avp_h[%d]=%x\n", x, tmp[x] );
    //    }

//	tmp = (unsigned char *)avp;
//	for (x=0;x<8;x++){
  //      log(LOG_WARN,"!!msg:::avp[%d]=%x\n", x, tmp[x] );
    //    }

	swaps (avp_h, SIZE_OF_AVP_HEADER);
	/* Go ahead and byte-swap the header */	

//	swaps (avp, sizeof(struct avp_hdr));
//	log (LOG_WARN,"!!!!!Debug msg:: The avp_h.length=%d\n", ALENGTH(avp_h->length));
 	super_safe_copy (avp_buf, (unsigned char *)avp, ALENGTH(avp_h->length)); 
	avp = avp_buf;
#if 0
	tmp = (unsigned char *)avp;
	for (x=0;x<8;x++){
	...
	tmp = (unsigned char *)avp_buf;
        for (x=0;x<8;x++){
        log(LOG_WARN,"#####msg:::avp_buf[%d]=%x\n", x, tmp[x] );
        }
	log(LOG_WARN,"MSG11111:::avp_buf=%x, avp=%x\n", avp_buf, avp);
	log(LOG_WARN,"MSG22222:::avp_buf=%x, avp=%x\n", &avp_buf, &avp);

#endif
	swaps (avp, SIZE_OF_AVP_HEADER);
//end SJ_Yen	
        if (avp->attr > AVP_MAX)
        {
            if (AMBIT (avp->length))
...
/*	    log (LOG_WARN, 
		 "\n\n!!!!the length of avp->length is %d, len = %d\n\n",
		  avp->length, len);
*/
            log (LOG_WARN,
...
//        if (ALENGTH (avp->length) < sizeof (struct avp_hdr))
	if(ALENGTH(avp->length) < SIZE_OF_AVP_HEADER)
        {
      ...
      next:
        ...
        {
//	    log (LOG_WARN,"Debug msg11:: The len =%d, avp->length=%d\n",len, ALENGTH(avp->length));
            len -= ALENGTH (avp->length);
//	    log (LOG_WARN,"Debug msg22:: The len =%d, avp->length=%d\n",len, ALENGTH(avp->length));
//	    log (LOG_WARN,"Debug msg33:: The data =%d, avp->length=%d\n",data, ALENGTH(avp->length));
            data += ALENGTH (avp->length);      /* Next AVP, please */
//	    log (LOG_WARN,"Debug msg44:: The data =%d, avp->length=%d\n",data, ALENGTH(avp->length));
        }
        avp = (struct avp_hdr *) data;
        firstavp = 0;
    }
