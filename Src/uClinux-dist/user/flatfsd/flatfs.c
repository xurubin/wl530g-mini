...
int flatwrite(char *flatfs)
{
	...
	struct flatent	ent;
	int             rc;
	time_t			total_flt_write_time, fs_read_time, flt_write_time;
	int log_level, ret;

	total_flt_write_time = time(NULL);
	...
	fs_read_time = time(NULL) - fs_read_time;
	flt_write_time = time(NULL);

	ret = flat_dev_close(0);

	flt_write_time = time(NULL) - flt_write_time;
	total_flt_write_time = time(NULL) - total_flt_write_time;

	log_level = LOG_ALERT;
	if (total_flt_write_time<=20) {
		log_level = LOG_DEBUG;
	} else if ((total_flt_write_time>20) && (total_flt_write_time<=40)) {
		log_level = LOG_NOTICE;
	} else if ((total_flt_write_time>40) && (total_flt_write_time<=100)){
		log_level = LOG_ERR;
	} else {
		log_level = LOG_ALERT;
	}
	syslog(log_level, "Write times are:%ld,%ld,%ld", 
		total_flt_write_time,
		fs_read_time,
		flt_write_time
		);
