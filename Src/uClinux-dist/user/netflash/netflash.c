...
#define CMD_LINE_OPTIONS "bc:Cd:fFhiHjklno:pr:stuv?" DECOMPRESS_OPTIONS HMACMD5_OPTIONS SETSRC_OPTIONS
...
static int get_segment(int rd, char *sgdata, int sgpos, int sgsize)
{
	...
	if (preserve || offset) {
		if (lseek(rd, sgpos, SEEK_SET) != sgpos) {
			...
			exit(1);
		} else if (read(rd, sgdata, preserve ? sgsize : offset) < 0) {
...
int netflashmain(int argc, char *argv[])
{
	...
	while ((rc = getopt(argc, argv, CMD_LINE_OPTIONS)) > 0) {
		...
		case 'n':
			...
			dochecksum = doversion = doremoveversion = 0;
			break;
		case 'o':
...
#if defined(CONFIG_ROMFS_FROM_ROM)
	...
	if (stat(rdev, &stat_buf) != 0) {
		error("open(%s)=%d failed (errno=%d)",
			rdev, rd, errno);
	} else if (S_ISBLK(stat_buf.st_mode)) {
#ifdef CONFIG_NFTL_RW
		if (major(stat_buf.st_rdev) == NFTL_MAJOR) {
			unsigned long l;
