...
static int l2tp_session_send_icrq(void *tun, void *sess)
{
	...
	bearer_type.value = 0;
	bearer_type.digital = session->config.bearer_type_digital;
	bearer_type.analog = session->config.bearer_type_analog;
...
static int l2tp_session_send_iccn(void *tun, void *sess)
{
	...
	framing_type.async = session->config.framing_type_async;
	framing_type.sync = session->config.framing_type_sync;
...
int l2tp_session_info_get(struct l2tp_session const *session, __u16 tunnel_id, __u16 session_id, struct l2tp_api_session_msg_data *result)
{
	...
	result->peer.framing_type_sync = session->peer.framing_type.sync;
	result->peer.framing_type_async = session->peer.framing_type.async;
	result->peer.bearer_type_digital = session->peer.bearer_type.digital;
	result->peer.bearer_type_analog = session->peer.bearer_type.analog;
