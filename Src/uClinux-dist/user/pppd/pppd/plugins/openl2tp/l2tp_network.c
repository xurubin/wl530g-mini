...
struct l2tp_control_hdr {
	union {
		struct {
			__u8 X(X(X(X(X(X(p_bit:1, o_bit:1), rsvd_2:1), s_bit:1), rsvd_1:2), l_bit:1), t_bit:1);
			__u8 X(ver:4, rsvd_3:4);
		} ufs;
		__u16 flags_ver;
	} uf;
	union {
...
static inline void l2tp_net_parse_header(struct l2tp_packet *pkt, __u16 *ver, int *is_data, int *has_seq, void **payload)
{
	struct l2tp_control_hdr* hdr = pkt->buf[0].data;

	__u16 offset = 0;
	__u16 dlen = 0;
	__u16 *optval = (__u16 *) &hdr->ud.data;

	*ver = (__u16) hdr->uf.ufs.ver;
	if (hdr->uf.ufs.t_bit) {
		*is_data = 0;
	} else {
		*is_data = 1;
...
int l2tp_net_build_header(void **buf, int *buf_len, __u16 ns, __u16 nr, __u16 tunnel_id, __u16 session_id)
{
	...
	lh->uf.ufs.o_bit = 0;
	lh->uf.ufs.ver = 2;
	lh->ud.uds.length = htons(sizeof(*lh));
	lh->ud.uds.nr = htons(nr);
	lh->ud.uds.ns = htons(ns);
	lh->ud.uds.tunnel_id = htons(tunnel_id);
	lh->ud.uds.session_id = htons(session_id);
...
void l2tp_net_update_header(void *buf, __u16 ns, __u16 nr)
{
	struct l2tp_control_hdr *lh;
...
int l2tp_net_send(struct l2tp_tunnel *tunnel, __u16 tunnel_id, __u16 session_id, struct l2tp_packet *pkt, __u8 msg_type)
{
	int result = 0;
	...
	l2tp_hdr->uf.flags_ver = 0;
	l2tp_hdr->uf.ufs.t_bit = -1;
	l2tp_hdr->uf.ufs.l_bit = -1;
	l2tp_hdr->uf.ufs.s_bit = -1;
	l2tp_hdr->uf.ufs.ver = 2;
	l2tp_hdr->ud.uds.tunnel_id = htons(tunnel_id);
	l2tp_hdr->ud.uds.session_id = htons(session_id);
	l2tp_hdr->ud.uds.ns = 0;
	l2tp_hdr->ud.uds.nr = 0;
	l2tp_hdr->ud.uds.length = htons(pkt->total_len);
