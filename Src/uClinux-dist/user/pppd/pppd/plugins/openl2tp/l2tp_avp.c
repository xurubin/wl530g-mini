...
struct l2tp_avp_info {
	int		type;		/* AVP type */
	__u16		vendor;		/* the vendor id associated w/ AVP */
	int		hide;		/* can the AVP be hidden? */
...
struct l2tp_avp_framing_cap 		l2tp_avp_my_framing_cap;
struct l2tp_avp_bearer_cap		l2tp_avp_my_bearer_cap;
...
static void l2tp_avp_log_packet(struct l2tp_avp_hdr *avp, struct l2tp_tunnel const *tunnel);
...
{
  /*   type			vend	hide	encode/decode funcs		name */
  { TYPE(MESSAGE),		0,	NO,	FUNCS(message_type),		"Message Type" },
  { TYPE(RESULT_CODE),		0,	NO,	FUNCS(result_code),		"Result Code" },
  { TYPE(PROTOCOL_VERSION),	0,	NO,	FUNCS(protocol_version),	"Protocol Version" },
  { TYPE(FRAMING_CAP),		0,	MAY,	FUNCS(framing_cap),		"Framing Capabilities" },
  { TYPE(BEARER_CAP),		0,	MAY,	FUNCS(bearer_cap),		"Bearer Capabilities" },
  { TYPE(TIEBREAKER),		0,	NO,	FUNCS(tiebreaker),		"Tie Breaker" },
  { TYPE(FIRMWARE_REVISION),	0,	MAY,	FUNCS(firmware_revision),	"Firmware Revision" },
  { TYPE(HOST_NAME),		0,	NO,	FUNCS(host_name),		"Host Name" },
  { TYPE(VENDOR_NAME),		0,	MAY,	FUNCS(vendor_name),		"Vendor Name" },
  { TYPE(TUNNEL_ID),		0,	MAY,	FUNCS(tunnel_id),		"Assigned Tunnel ID" },
  { TYPE(RX_WINDOW_SIZE),	0,	NO,	FUNCS(rx_window_size),		"Receive Window Size" },
  { TYPE(CHALLENGE),		0,	MAY,	FUNCS(challenge),		"Challenge" },
  { TYPE(Q931_CAUSE_CODE),	0,	NO,	FUNCS(q931_cause_code),		"Q.931 Cause Code" },
  { TYPE(CHALLENGE_RESPONSE),	0,	MAY,	FUNCS(challenge_response),	"Challenge Response" },
  { TYPE(SESSION_ID),		0,	MAY,	FUNCS(session_id),		"Assigned Session ID" },
  { TYPE(CALL_SERIAL_NUMBER),	0,	MAY,	FUNCS(call_serial_number),	"Call Serial Number" },
  { TYPE(MINIMUM_BPS),		0,	MAY,	FUNCS(minimum_bps),		"Minimum BPS" },
  { TYPE(MAXIMUM_BPS),		0,	MAY,	FUNCS(maximum_bps),		"Maximum BPS" },
  { TYPE(BEARER_TYPE),		0,	MAY,	FUNCS(bearer_type),		"Bearer Type" },
  { TYPE(FRAMING_TYPE),		0,	MAY,	FUNCS(framing_type),		"Framing Type" },
  { TYPE(PACKET_PROC_DELAY),	0,	NO,	FUNCS(packet_proc_delay),	"Packet Process Delay" },
  { TYPE(CALLED_NUMBER),	0,	MAY,	FUNCS(called_number),		"Dialed Number" },
  { TYPE(CALLING_NUMBER),	0,	MAY,	FUNCS(calling_number),		"Dialing Number" },
  { TYPE(SUB_ADDRESS),		0,	MAY,	FUNCS(sub_address),		"Sub-Address" },
  { TYPE(CONNECT_SPEED),	0,	MAY,	FUNCS(tx_connect_speed),	"Transmit Connect Speed" },
  { TYPE(PHYSICAL_CHANNEL_ID),	0,	MAY,	FUNCS(physical_channel_id),	"Physical channel ID" },
  { TYPE(INITIAL_RCVD_LCP_CONFREQ), 0,	MAY,	FUNCS(initial_lcp_confreq),	"Initial Received LCP Confreq" },
  { TYPE(LAST_SENT_LCP_CONFREQ),0,	MAY,	FUNCS(last_sent_lcp_confreq),	"Last Sent LCP Confreq" },
  { TYPE(LAST_RCVD_LCP_CONFREQ),0,	MAY,	FUNCS(last_rcvd_lcp_confreq),	"Last Received LCP Confreq" },
  { TYPE(PROXY_AUTH_TYPE),	0,	MAY,	FUNCS(proxy_auth_type),		"Proxy Authen Type" },
  { TYPE(PROXY_AUTH_NAME),	0,	MAY,	FUNCS(proxy_auth_name),		"Proxy Authen Name" },
  { TYPE(PROXY_AUTH_CHALLENGE),	0,	MAY,	FUNCS(proxy_auth_chall),	"Proxy Authen Challenge" },
  { TYPE(PROXY_AUTH_ID),	0,	MAY,	FUNCS(proxy_auth_id),		"Proxy Authen ID" },
  { TYPE(PROXY_AUTH_RESPONSE),	0,	MAY,	FUNCS(proxy_auth_rsp),		"Proxy Authen Response" },
  { TYPE(CALL_ERRORS),		0,	MAY,	FUNCS(call_errors),		"Call Errors" },
  { TYPE(ACCM),			0,	MAY,	FUNCS(accm),			"ACCM" },
  { TYPE(RANDOM_VECTOR),	0,	NO,	FUNCS(random_vector),		"Random Vector" },
  { TYPE(PRIV_GROUP_ID),	0,	MAY,	FUNCS(priv_group_id),		"Private Group ID" },
  { TYPE(RX_CONNECT_SPEED),	0,	MAY,	FUNCS(rx_connect_speed),	"Receive Connect Speed" },
  { TYPE(SEQUENCING_REQUIRED),	0,	NO,	FUNCS(sequencing_required),	"Sequencing Required" },
};
...
struct l2tp_msg_info {
	...
	__u64		mandatory; 	/* bit set at (1 << avp_type) implies mandatory */
	__u64		optional; 	/* bit set at (1 << avp_type) implies optional */
	const char	*name;		/* for debugging */
};
			   ...
			   unsigned char const *random_vector, int random_vector_len)
{
	...
	__u16 *orig_len = buffer + L2TP_AVP_HEADER_LEN;
	__u16 orig_buffer_len;
	int result;
	__u16 flag_len;
...
static void l2tp_avp_hdr_write(struct l2tp_avp_hdr *hdr, 
			       struct l2tp_msg_info const *msg_info, 
			       const __u16 avp_type, const int var_len, int hide)
{
	__u16 flag_len;

	flag_len = L2TP_AVP_HDR_LEN(L2TP_AVP_HEADER_LEN + var_len);
	if (msg_info->mandatory & (1 << avp_type)) {
	...
	hdr->vendor_id = 0;
	hdr->type = __constant_htons(avp_type);
	hdr->flag_len = htons(flag_len);
}
...
{
	__u16 flag_len = ntohs(hdr->flag_len);
				       ...
				       struct l2tp_tunnel const *tunnel, int hide)
{
	...
	avp_data->uv.value = htonl(data->value->framing_cap.uv.value);
	return result;
}
				       ...
				       struct l2tp_avp_desc *result, struct l2tp_tunnel const *tunnel)
{
	...
	result->value = (void *) &hdr->value[0];
	result->value_len = avp_len - L2TP_AVP_HEADER_LEN;
	result->value->framing_cap.uv.value = ntohl(result->value->framing_cap.uv.value);
	l2tp_tunnel_log(tunnel, L2TP_AVPDATA, LOG_DEBUG, "AVPDATA: FRAMING_CAP: cap=%hu", result->value->framing_cap.uv.value);
	return avp_len;
}
				      ...
				      struct l2tp_tunnel const *tunnel, int hide)
{
	...
	avp_data->uv.value = htonl(data->value->bearer_cap.uv.value);
	return result;
}
				      ...
				      struct l2tp_avp_desc *result, struct l2tp_tunnel const *tunnel)
{
	...
	result->value = (void *) &hdr->value[0];
	result->value_len = avp_len - L2TP_AVP_HEADER_LEN;
	result->value->bearer_cap.uv.value = ntohl(result->value->bearer_cap.uv.value);
	l2tp_tunnel_log(tunnel, L2TP_AVPDATA, LOG_DEBUG, "AVPDATA: BEARER_CAP: cap=%hu", result->value->bearer_cap.uv.value);
	return avp_len;
}
				       ...
				       struct l2tp_tunnel const *tunnel, int hide)
{
	...
	avp_data->uv.value = htonl(data->value->bearer_type.uv.value);
	return result;
}
				       ...
				       struct l2tp_avp_desc *result, struct l2tp_tunnel const *tunnel)
{
	...
	result->value = (void *) &hdr->value[0];
	result->value_len = avp_len - L2TP_AVP_HEADER_LEN;
	result->value->bearer_type.uv.value = ntohl(result->value->bearer_type.uv.value);
	l2tp_tunnel_log(tunnel, L2TP_AVPDATA, LOG_DEBUG, "AVPDATA: BEARER_TYPE: type=%u", result->value->bearer_type.uv.value);
	return avp_len;
}
					...
					struct l2tp_tunnel const *tunnel, int hide)
{
	...
	avp_data->uv.value = htonl(data->value->framing_type.uv.value);
	return result;
}
					...
					struct l2tp_avp_desc *result, struct l2tp_tunnel const *tunnel)
{
	...
	result->value = (void *) &hdr->value[0];
	result->value_len = avp_len - L2TP_AVP_HEADER_LEN;
	result->value->framing_type.uv.value = ntohl(result->value->framing_type.uv.value);
	l2tp_tunnel_log(tunnel, L2TP_AVPDATA, LOG_DEBUG, "AVPDATA: FRAMING_TYPE: type=%u", result->value->framing_type.uv.value);
	return avp_len;
}
...
unsupported_message:
	...
	l2tp_avp_log_packet(first_avp, tunnel);
 ...
 */
int l2tp_avp_message_encode(__u16 msg_type, struct l2tp_packet **ppkt, int hide, 
 ...
 * message fully.
 */
int l2tp_avp_preparse(struct l2tp_avp_hdr *first_avp, int avp_data_len, char **host_name, __u16 *msg_type)
{
...
static void l2tp_avp_log_packet(struct l2tp_avp_hdr *avp, struct l2tp_tunnel const *tunnel)
{
	if (l2tp_tunnel_get_trace_flags(tunnel) & L2TP_DATA) {
		__u16 flag_len = ntohs(avp->flag_len);
...
void l2tp_avp_init(void)
{
	...
	l2tp_avp_my_framing_cap.uv.value = 0;
	l2tp_avp_my_framing_cap.uv.sync = 1;
	l2tp_avp_my_bearer_cap.uv.value = 0;
	l2tp_avp_my_bearer_cap.uv.digital = 1;
}
