...
{
	__u16 my_id1 = *(__u16 *) id1;
	__u16 my_id2 = *(__u16 *) id2;
...
static void ip_to_string(char *result, __u32 addr)
{
	struct in_addr ip;
	char *str;
...
static int l2tp_act_ppp_profile_show(struct cli_node *node, int argc, char *argv[], int *arg_num)
{
	...
	printf("    local address: %s, peer address: %s\n"
	       "    dns addresses: %s / %s\n"
	       "    wins addresses: %s / %s\n",
	       local_ip, peer_ip, dns1, dns2, wins1, wins2);
...
static int l2tp_config_tunnel_map_add(char *tunnel_name, __u16 tunnel_id)
{
...
static struct l2tp_config_tunnel_map *l2tp_config_tunnel_map_find(char *tunnel_name, __u16 tunnel_id)
{
	struct l2tp_config_tunnel_map *entry;
