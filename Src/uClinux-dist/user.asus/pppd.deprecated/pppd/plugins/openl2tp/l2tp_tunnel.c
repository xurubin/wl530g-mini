...
struct l2tp_tunnel {
	 ...
	 */
	struct {
		__u32				flags;				/* remembers which fields have been set */
		__u32				flags2;
		...
		int				hello_timeout;
		int				max_retries;
		__u16				rx_window_size;
		__u16				tx_window_size;
		int				retry_timeout;
		int				idle_timeout;
	...
	struct {
		__u16				tunnel_id;			/* assigned tunnel id */
...
int (*l2tp_tunnel_created_hook)(__u16 tunnel_id) = NULL;
int (*l2tp_tunnel_deleted_hook)(__u16 tunnel_id) = NULL;
int (*l2tp_tunnel_modified_hook)(__u16 tunnel_id) = NULL;
int (*l2tp_tunnel_up_hook)(__u16 tunnel_id, __u16 peer_tunnel_id) = NULL;
int (*l2tp_tunnel_down_hook)(__u16 tunnel_id) = NULL;
...
static inline struct usl_hlist_head *l2tp_tunnel_id_hash(__u16 tunnel_id)
{
...
{
	unsigned hash = usl_hash_full_name(tunnel_name, strlen(tunnel_name));
	return &l2tp_tunnel_name_list[hash & (L2TP_TUNNEL_HASH_SIZE - 1)];
}

static inline struct usl_hlist_head *l2tp_tunnel_session_id_hash(struct l2tp_tunnel *tunnel, __u16 session_id)
{
...
struct usl_hlist_head *l2tp_tunnel_session_id_hlist(struct l2tp_tunnel *tunnel, __u16 session_id)
{
	return l2tp_tunnel_session_id_hash(tunnel, session_id);
}

__u16 l2tp_tunnel_id(struct l2tp_tunnel const *tunnel)
{
	return tunnel->status.tunnel_id;
}

__u16 l2tp_tunnel_peer_id(struct l2tp_tunnel const *tunnel)
{
...
struct l2tp_tunnel *l2tp_tunnel_find_by_id(__u16 tunnel_id)
{
 ...
 */
static __u16 l2tp_tunnel_allocate_id(void)
{
	__u16 tunnel_id;
	int tries;
	struct l2tp_tunnel *tunnel;
 ...
 */
static void l2tp_tunnel_set_result(struct l2tp_tunnel *tunnel, __u16 result_code,
						 ...
						 struct l2tp_avp_challenge_response *challenge_response)
{
	...
	__u8 *digest = &challenge_response->value[0];
	__u8 chap_id = id;
 ...
 */
void l2tp_tunnel_session_add_again(struct l2tp_tunnel *tunnel, struct usl_hlist_node *hlist, __u16 session_id)
{
	usl_hlist_add_head(hlist, l2tp_tunnel_session_id_hash(tunnel, session_id));
	tunnel->status.num_sessions++;
}

int l2tp_tunnel_session_add(struct l2tp_tunnel *tunnel, struct usl_list_head *list, struct usl_hlist_node *hlist, __u16 session_id)
{
	int result = 0;
...
static void l2tp_tunnel_act_send_sccrq(void *tun, void *session, void *arg3)
{
	...
	avps[L2TP_AVP_TYPE_FRAMING_CAP].value = (void *) &l2tp_avp_my_framing_cap;
	avps[L2TP_AVP_TYPE_FRAMING_CAP].value_len = sizeof(l2tp_avp_my_framing_cap);
	...
	avps[L2TP_AVP_TYPE_BEARER_CAP].value = (void *) &l2tp_avp_my_bearer_cap;
	avps[L2TP_AVP_TYPE_BEARER_CAP].value_len = sizeof(l2tp_avp_my_bearer_cap);
...
static int l2tp_tunnel_store_avps(struct l2tp_tunnel *tunnel, struct l2tp_avp_desc *avps)
{
	...
	if (avps[L2TP_AVP_TYPE_FRAMING_CAP].value != NULL) {
		tunnel->peer.framing_cap.uv.value = avps[L2TP_AVP_TYPE_FRAMING_CAP].value->framing_cap.uv.value;
	}
	if (avps[L2TP_AVP_TYPE_BEARER_CAP].value != NULL) {
		tunnel->peer.bearer_cap.uv.value = avps[L2TP_AVP_TYPE_BEARER_CAP].value->bearer_cap.uv.value;
	}
 ...
 */
void l2tp_tunnel_recv(struct l2tp_tunnel *tunnel, __u16 tunnel_id, 
...
int l2tp_tunnel_queue_event(__u16 tunnel_id, int event)
{
	struct l2tp_tunnel_event_msg msg;
	int result;
...
static void l2tp_tunnel_recreate(struct l2tp_tunnel *tunnel)
{
	...
	/* Find the new tunnel */
	new_tunnel_id = (__u16) (result & 0xffff);
...
struct l2tp_tunnel *l2tp_tunnel_alloc(__u16 tunnel_id, char *tunnel_profile_name, char *peer_profile_name, int created_by_admin, int *ret)
{
 ...
 */
static struct l2tp_tunnel *l2tp_tunnel_get_instance(__u16 tunnel_id, char *tunnel_name)
{
	struct l2tp_tunnel *tunnel = NULL;
...
bool_t l2tp_tunnel_get_1_svc(u_short tunnel_id, optstring tunnel_name, struct l2tp_api_tunnel_msg_data *result, struct svc_req *req)
{
	...
	result->peer.framing_cap_sync = tunnel->peer.framing_cap.uv.sync;
	result->peer.framing_cap_async = tunnel->peer.framing_cap.uv.async;
	result->peer.bearer_cap_digital = tunnel->peer.bearer_cap.uv.digital;
	result->peer.bearer_cap_analog = tunnel->peer.bearer_cap.uv.analog;
