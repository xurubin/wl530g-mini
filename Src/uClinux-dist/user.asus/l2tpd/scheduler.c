...
void alarm_handler (int signal)
{
	...
	if (strlen(lns_server_name) != 0){ //SJ_Yen
		cnt++;}
    cnt++;
//     log (LOG_DEBUG, "alarm_handler : The cnt value is %d\n", cnt);
    if (cnt != 1)
    {
...
//	 log (LOG_DEBUG, "%s: Start before while(events)\n",
//        __FUNCTION__);
    while (events)
    {
...
//	 log (LOG_DEBUG, "%s: Start before if(events)\n",
//        __FUNCTION__);
    if (events)
    {
...
//	alarm_handler(0); 
//    log (LOG_DEBUG, "%s: Start before while(sigprocmask...)\n",
//	__FUNCTION__);
