...
/*                                      c->ourrws = c->lac->call_rws;
	...
	case SCCRQ:
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer did not specify assigned tunnel ID.  Closing.\n", __FUNCTION__);
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Denied connection to unauthorized peer %s\n", __FUNCTION__, IPADDY(t->peer.sin_addr));
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer did not specify framing capability.  Closing.\n", __FUNCTION__);
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer did not specify hostname.  Closing.\n", __FUNCTION__);
		...
		while (y)
		{
			if ((y->tid == t->tid) && (y->peer.sin_addr.s_addr == t->peer.sin_addr.s_addr) && (y != t))
			{
	...
	case SCCRP:
		...
		t->hello = schedule(tv, hello, (void *) t);
		dbg ("1\n"); //eric++
		log(LOG_NOTICE, "Connection established to %s, %d.  Local: %d, Remote: %d.\n", IPADDY(t->peer.sin_addr), ntohs(t->peer.sin_port), t->ourtid, t->tid);
//SJ_Yen
		sprintf(lns_server_name, "c %d", t->ourtid);
		log(LOG_NOTICE, "Debug msg:::The string send to lns_server_name = '%s'.\n", lns_server_name);
		log(LOG_NOTICE, "Debug msg:::The length of the string send to lns_server_name is '%d'\n", strlen(lns_server_name));
	...
	case SCCCN:
		...
		dbg ("2\n"); //eric++
		log(LOG_NOTICE, "Connection established to %s, %d.  Local: %d, Remote: %d.  LNS session is '%s'\n", IPADDY(t->peer.sin_addr), ntohs(t->peer.sin_port), t->ourtid, t->tid, t->lns->entname);
	...
	case StopCCN:
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to disconnect without specifying tunnel ID\n", __FUNCTION__);
			return -EINVAL;
		}
		if ((t->qtid != t->tid) && (t->tid > 0))
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to disconnect with invalid TID (%d != %d)\n", __FUNCTION__, t->qtid, t->tid);
			return -EINVAL;
		}
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to disconnect without specifying result code.\n", __FUNCTION__);
			return -EINVAL;
		}
		dbg ("3\n"); //eric++
		log(LOG_LOG, "%s: Connection closed to %s, port %d (%s), Local: %d, Remote: %d\n", __FUNCTION__, IPADDY(t->peer.sin_addr), ntohs(t->peer.sin_port), t->self->errormsg, t->ourtid, t->tid);
		c->needclose = 0;
		c->closing = -1;
		break;
	case ICRQ:
...
/*		p->ourrws = p->lns->call_rws;
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to initiate call without call ID\n", __FUNCTION__);
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer did not specify serial number when initiating call\n", __FUNCTION__);
			call_close(p);
			return -EINVAL;
		}
#endif
...
/*		if (p->ourrws >=0)
	...
	case ICRP:
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to negotiate ICRP without specifying call ID\n", __FUNCTION__);
			c->needclose = -1;
			return -EINVAL;
		}
		c->state = ICCN;
...
/*		if (c->ourrws >= 0)
		...
		dbg ("4\n"); //eric++
		log(LOG_NOTICE, "Call established with %s, Local: %d, Remote: %d, Serial: %d\n", IPADDY(t->peer.sin_addr), c->ourcid, c->cid, c->serno);
		...
		po = add_opt(po, "file");
		po = add_opt(po, "/tmp/l2tp/options.l2tpd");
/*SJ_Yen
	po = add_opt (po, "file");
...
// end SJ_Yen*/
...
#ifdef IP_ALLOCATION
			po = add_opt(po, "%s:%s", c->lac->localaddr ? ip1 : "", c->lac->remoteaddr ? ip2 : "");
#endif
			if (c->lac->authself)
			{
				if (c->lac->pap_refuse)
	...
	case ICCN:
		if (c == t->self)
		{
			log(LOG_DEBUG, "%s: Peer attempted ICCN on the actual tunnel, not the call", __FUNCTION__);
			return -EINVAL;
		}
		if (c->txspeed < 1)
		{
			log(LOG_DEBUG, "%s: Peer did not specify transmit speed\n", __FUNCTION__);
			c->needclose = -1;
			return -EINVAL;
		};
		if (c->frame < 1)
		{
			log(LOG_DEBUG, "%s: Peer did not specify framing type\n", __FUNCTION__);
			c->needclose = -1;
			return -EINVAL;
		}
		c->state = ICCN;
	...
	case CDN:
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to disconnect without specifying call ID\n", __FUNCTION__);
			return -EINVAL;
		}
		if (c == t->self)
		{
			...
			{
				if (DEBUG)
					log(LOG_DEBUG, "%s: Unable to determine call to be disconnected.\n", __FUNCTION__);
				return -EINVAL;
			}
		} else
			p = c;
		...
		{
			if (DEBUG)
				log(LOG_DEBUG, "%s: Peer tried to disconnect with invalid CID (%d != %d)\n", __FUNCTION__, c->qcid, c->cid);
			return -EINVAL;
		}
		c->qcid = -1;
		if (c->result < 0)
		...
		log(LOG_LOG, "%s: Connection closed to %s, serial %d (%s)\n", __FUNCTION__, IPADDY(t->peer.sin_addr), c->serno, c->errormsg);
		c->needclose = 0;
		c->closing = -1;
		break;
	case Hello:
	...
	case SLI:
		break;
	default:
		log(LOG_DEBUG, "%s: Don't know how to finish a message of type %d\n", __FUNCTION__, c->msgtype);
...
inline int check_control(const struct buffer *buf, struct tunnel *t, struct call *c)
{
	...
	if (buf->len < sizeof(struct control_hdr))
	{
		if (DEBUG)
		{
			log(LOG_DEBUG, "%s: Received too small of packet\n", __FUNCTION__);
		}
		return -EINVAL;
	}
#ifdef SANITY
	if (buf->len != h->length)
	{
		if (DEBUG)
		{
			log(LOG_DEBUG, "%s: Reported and actual sizes differ (%d != %d)\n", __FUNCTION__, h->length, buf->len);
		}
		return -EINVAL;
	}
	/*
	...
	{
		if (DEBUG)
			log(LOG_DEBUG, "%s: Received out of order control packet on tunnel %d (%d != %d)\n", __FUNCTION__, t->tid, h->Ns, t->control_rec_seq_num);
		if (((h->Ns < t->control_rec_seq_num) && ((t->control_rec_seq_num - h->Ns) < 32768)) || ((h->Ns > t->control_rec_seq_num) && ((t->control_rec_seq_num - h->Ns) > 32768)))
		{
			/*
...
inline int expand_payload(struct buffer *buf, struct tunnel *t, struct call *c)
{
	...
	if (new_hdr->Ns != c->data_seq_num)
	{
		...
		if (((new_hdr->Ns < c->data_rec_seq_num) && ((c->data_rec_seq_num - new_hdr->Ns) < 32768)) || ((new_hdr->Ns > c->data_rec_seq_num) && ((c->data_rec_seq_num - new_hdr->Ns) > 32768)))
		{
#ifdef DEBUG_FLOW
			if (DEBUG)
				log(LOG_DEBUG, "%s: Already seen this packet before (%d < %d)\n", __FUNCTION__, new_hdr->Ns, c->pSr);
#endif
			return -EINVAL;
...
#ifdef DEBUG_FLOW
			if (DEBUG)
				log(LOG_DEBUG, "%s: Oops, lost a packet or two (%d != %d).  continuing...\n", __FUNCTION__, new_hdr->Ns, c->pSr);
#endif
...
#ifdef DEBUG_FLOW
			if (DEBUG)
				log(LOG_DEBUG, "%s: Received out of order payload packet (%d != %d)\n", __FUNCTION__, new_hdr->Ns, c->pSr);
#endif
			return -EINVAL;
		}
	} else
		...
		if (!check_payload(buf, t, c))
		{
			if (!expand_payload(buf, t, c))
			{
				if (buf->len > sizeof(struct payload_hdr))
				{
					...
					M res = write_packet(buf, t, c, c->frame & SYNC_FRAMING);
#endif
