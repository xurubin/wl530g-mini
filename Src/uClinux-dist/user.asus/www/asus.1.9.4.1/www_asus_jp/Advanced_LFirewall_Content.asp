...
<td class="content_header_td_title" colspan="2">インターネットファイアウォール - t2WZFilter</td>
</tr>
<tr>
<td class="content_desc_td" colspan="2">WAN vs. ZVMODELVZフィルタは、WANとZVMODELVZ間の指定されたパケットをブロックします。はじめに、両方の方向にあるフィルタに対するデフォルトの動作を選択できます。次に、すべての例外に対する規則を挿入してください。
         </td>
</tr>
<tr>
<td class="content_header_td">WAN vs. ZVMODELVZフィルタを有効にしますか?
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、WAN対ZVMODELVZフィルタが有効になる日付を定義します。', LEFT);" onMouseOut="return nd();">WAN対ZVMODELVZフィルタが有効になる日付:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、WAN対ZVMODELVZフィルタが有効になる時間間隔を定義します。', LEFT);" onMouseOut="return nd();">WAN対ZVMODELVZフィルタが有効になる日時:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、WAN対ZVMODELVZフィルタ表で指定されていないWAN対ZVMODELVZパケットが受け入れられるか脱落するかを定義します。', LEFT);" onMouseOut="return nd();">指定されていないパケット(WAN対ZVMODELVZ):
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、フィルタされるWAN対ZVMODELVZ ICMPパケットタイプのリストを定義します。例えば、Echo(タイプ8)とEcho Reply(タイプ0) ICMPパケットをフィルタしたい場合、0 5などの、空白で区切られる数字を持つ文字列を入力する必要があります。', LEFT);" onMouseOut="return nd();">フィルタされるICMP(WAN対ZVMODELVZ)パケットタイプ:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、ZVMODELVZとWAN間でどのタイプのパケットがログされるかを示します。', LEFT);" onMouseOut="return nd();">ZVMODELVZとWANの間のログタイプ:
...
<td class="content_list_header_td" width="60%" id="WLocalFilterList"> WANと ZVMODELVZ フィルタ表
         <input type="hidden" name="WanLocalRuleCount_0" value="<% nvram_get_x("FirewallConfig", "WanLocalRuleCount"); %>" readonly="1"></td><td width="10%">
<div align="center">
<input class="inputSubmit" type="submit" onMouseOut="buttonOut(this)" onMouseOver="buttonOver(this)" onClick="return markGroup(this, 'WLocalFilterList', 32, ' Add ');" name="WLocalFilterList" value="追加" size="12">
</div>
</td><td width="10%">
<div align="center">
<input class="inputSubmit" type="submit" onMouseOut="buttonOut(this)" onMouseOver="buttonOver(this)" onClick="return markGroup(this, 'WLocalFilterList', 32, ' Del ');" name="WLocalFilterList" value="削除" size="12">
</div>
</td><td width="10%">
<div align="center">
<input class="inputSubmit" type="button" onMouseOut="buttonOut(this)" onMouseOver="buttonOver(this)" onClick="return openHelp(this, 'FilterHelp');" name="WLocalFilterList" value="ヘルプ" size="12">
</div>
</td><td width="5%"></td>
</tr>
