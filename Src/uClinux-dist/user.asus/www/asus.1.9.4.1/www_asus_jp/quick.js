...
{    	
    ...
    {
    	if (confirm("新しいワイヤレス認証と暗号化を選択しました。既存の設定に上書きますか。"))
    	{
...
{			
	...
	{	
	    if (document.form.x_WANType[1].checked == true || document.form.x_WANType[2].checked == true)
	    {
	       ...
	       {
	    	   alert("ADSLモデムへの接続を選択する場合、これらの2つのフィールドをブランクにできません。");
	    	   return;
	       }	      
	       // if (document.form.x_WANType[1].checked == true)
	       //{
        ...
        else if (s=='WANSetting')
	{
	    ...
	    {	        
	        alert("ルータモードでは、スタティックIPアドレスをシステムに割り当てる必要があります!");
	        return;		
	    }		    
...
{   
    if (document.form.SecurityLevel.value=="1") return(is_wlphrase(s, 'wl_phrase_x', o));
    else return(is_wlphrase(s, 'wl_wpa_psk', o));
}

function validate_wlphrase_q(s, o)
{   	      	   	   	
    if (document.form.SecurityLevel.value=="1") return(validate_wlphrase(s, 'wl_phrase_x', o));
