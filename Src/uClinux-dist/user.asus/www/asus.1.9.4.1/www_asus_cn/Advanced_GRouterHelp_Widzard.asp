...
<table border="1" width="542">
  ...
  <tr>
    ...
    <p><font size="4"><b>输入格式</b></font></p>        
      ...
      <li>
        <b>网络掩码</b> : 它可指出网络号（network ID）及子网号（subnet ID）所使用的字节数。例如：假设点十进制（dotted-decimal）网络掩码是255.255.255.0，那么它的网络掩码就是24位。假设该目的为一主机，那么它的网络掩码应该就是32位。
      </li> 
      <li>
        <b>网关</b> : 它代表的是数据包的传输目的地即网关的IP地址。数据包必须最先到达该网关。这意味着您必须事先设置好至该网关的静态路由。
      </li>        
       <li>
        <b>顺序</b> : 它代表路由规则的先后顺序。
      </li>  
       <li>
        <b>界面</b> : 它代表路由规则所遵循的网络界面。
      </li>
