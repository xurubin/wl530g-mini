...
<p></p>
从专业术语的角度解释，网关模式是 -启用网络地址转换（Network Address Translation，NAT），允许使用PPPoE、DHCP 用户端或静态IP来进行广域网（WAN）连接操作。另外，也支持一些对家庭用户相当有用的功能，像是UPNP及动态DNS（DDNS）。
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode" value=""> 路由器</td>
<td class="content_desc_td">
在「路由器」模式中，我们建议您使用ZVMODELVZ来连接您公司内部的局域网（LAN）。现在，您可设置路由协议以符合您的办公环境需求。
<p></p>
从专业术语的角度解释，路由器模式：可取消网络地址转换（NAT），允许设置静态路由协议。
</td>
</tr>
<tr>
...
<p></p>
从专业术语的角度解释，接入点（AP）模式：关闭网络地址转换（NAT），允许将ZVMODELVZ的一组互联网端口（WAN port）与四组局域网端口（LAN port）桥接在一起。
</td>
</tr>
<tr>
