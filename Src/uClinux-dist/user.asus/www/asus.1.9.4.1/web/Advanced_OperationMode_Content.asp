...
ZVMODELVZ支援三種可滿足不同需求的操作模式。請選擇能符合您現況的模式。
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode"> 家用閘道器</td>
<td class="content_desc_td">
在此種模式中，我們建議您使用ZVMODELVZ透過ADSL或Cable Modem來連結網際網路。同時，在您的環境中許多人都可分享同一IP來使用ISP。
<p></p>
從專業術語的角度解釋，閘道器模式是 -啟用網路位址轉換（Network Address Translation，NAT），允許使用PPPoE、DHCP client或靜態IP來進行網際網路（WAN）連線作業。另外，也支援一些對家庭用戶相當有用的功能，像是通用隨插即用（UPnP）及動態DNS（DDNS）。
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode" value=""> 路由器</td>
<td class="content_desc_td">
在「路由器」模式中，我們建議您使用ZVMODELVZ來連上您公司內部的區域網路（LAN）。現在，您便可設定路由通訊協定以符合您的辦公環境需求。
<p></p>
從專業術語的角度解釋，路由器模式是 - 可設定取消網路位址轉換（NAT），允許設定靜態路由通訊協定。
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode" value="V3"> 無線網路橋接器</td>
<td class="content_desc_td">
在「無線網路橋接器」模式中，五組乙太網路通訊埠（Ethernet ports）及無線通訊裝置全數裝設在同一區域網域（LAN）之中。在這裏不支援與WAN相關的功能。
<p></p>
從專業術語的角度解釋，無線網路橋接器模式是 - 取消網路位址轉換（NAT），允許將ZVMODELVZ的一組網際網路通訊埠（WAN port）與四組區域網路通訊埠（LAN port）橋接在一起。
</td>
</tr>
<tr>
