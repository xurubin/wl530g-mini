...
<tr>
<td class="content_header_td" onMouseOver="return overlib('在移除您的USB磁盘之前，请按下此键将缓存资料写回到磁盘上并等候本页面再次显示。', LEFT);" onMouseOut="return nd();">强制退出USB磁盘：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目指出了FTP通讯协议所使用的端口编号。', LEFT);" onMouseOut="return nd();">FTP端口：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目可让您在用户输入登入名称一段特定的时间后终止用户连接。', LEFT);" onMouseOut="return nd();">以秒计算的登入超时时间：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目可让您在用户登入却维持闲置一段特定的时间后终止用户连接。', LEFT);" onMouseOut="return nd();">以秒计算的维持超时时间：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('当您接上USB硬盘后，WL530g 会在此硬盘的根目录寻找您指定的文件并且执行。所以此栏目空白将会自动关闭这一个功能。', LEFT);" onMouseOut="return nd();">自动执行程序:
