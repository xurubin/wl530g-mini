...
<tr>
<td class="content_header_td" onMouseOver="return overlib('选择「Yes」时，可将WL530g连接至「远端桥接点列表」中所列的其他AP。', LEFT);" onMouseOut="return nd();">连接「远端桥接点列表」中的AP？
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('选择「Yes」时，会取消无线桥接限制。任何的无线桥接器均可准许存取。选择「No」时，会启用无线桥接限制。只有「远端桥接点列表」（ Remote Bridge List）中所列的桥接器才准予存取。', LEFT);" onMouseOut="return nd();">允许匿名？
