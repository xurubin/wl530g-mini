...
{    	
    ...
    {
    	if (confirm("您已选择另一种无线认证方式及加密功能，您确定要覆盖吗？"))
    	{
...
*/
    ...
    if (!sameWireless)
    {
       if (confirm('您的802.11a设置与802.11b的设置不同，您要将相同的设置同时应用在两组界面上吗？否则只须按下「取消」忽略无线设置。'))
       {	
...
{			
	...
	{	
	    if (document.form.x_WANType[1].checked == true || document.form.x_WANType[2].checked == true)
	    {
	       ...
	       {
	    	   alert("如果您选择连接至一台ADSL调制解调器，您便不能将此两个栏目保留空白。");
	    	   return;
	       }	      
	       // if (document.form.x_WANType[1].checked == true)
	       //{
