...
ZVMODELVZ支持两种满足不同需求的操作模式。请选择能符合您现状的模式。
</td>
</tr>
...
在「AP接入点」模式中，以太网络端口及无线设备均设置在同一局域网络（LAN）中。在这里不支持与WAN相关的功能。
<p></p>
从专业术语的角度解释，AP接入点模式是 - 关闭网络地址转换（NAT），允许将ZVMODELVZ的一组局域网端口及无线端口桥接在一起。
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode"> 家用网关</td>
<td class="content_desc_td">
在此种模式中，我们建议您只用ZVMODELVZ的以太网络端口，通过ADSL或Cable Modem来连接互联网。同时，在您的环境中许多人都可共享同一IP来使用ISP。
<p></p>
从专业术语的角度解释，网关模式是 - 启用网络地址转换（NAT），允许使用PPPoE、DHCP 用户端，或静态IP来进行广域网（WAN）连接操作。另外，也支持一些对家庭用户相当有用的功能，像是即插即用（UPnP）及动态DNS（DDNS）。
</td>
</tr>
<tr>
