...
{    	
    ...
    {
    	if (confirm("您已選擇另一種無線驗證方式及加密功能，您確定要作覆蓋嗎？"))
    	{
...
*/
    ...
    if (!sameWireless)
    {
       if (confirm('您的802.11a設定值與802.11b的設定值不同，您要將相同的設定值同時套用在兩組介面上嗎？否則只須按下「取消」略過無線設定值乙項。'))
       {	
...
{			
	...
	{	
	    if (document.form.x_WANType[1].checked == true || document.form.x_WANType[2].checked == true)
	    {
	       ...
	       {
	    	   alert("如果您選擇連接至一台ADSL數據機，您便不能將此兩個欄位保留空白。");
	    	   return;
	       }	      
	       // if (document.form.x_WANType[1].checked == true)
	       //{
