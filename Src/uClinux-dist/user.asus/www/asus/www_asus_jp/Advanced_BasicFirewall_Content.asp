...
<td class="content_header_td_title" colspan="2">インターネットファイアウォール - 基本的なコンフィグ</td>
</tr>
<tr>
<td class="content_desc_td" colspan="2">ファイアウォール(SPI Firewall)を有効にすると、ZVMODELVZとその背後にあるデバイスに対して基礎的保護を提供します。指定されたパケットを除去するには、次ページのWAN対LANフィルタを使用してください。
         </td>
</tr>
<tr>
<td class="content_header_td">ファイアウォールを有効にしますか?
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、LANとWAN間でログされるパケットの種類を示します。', LEFT);" onMouseOut="return nd();">ログされたパケットタイプ:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('この機能により、インターネットからZVMODELVZを管理できます。ホームゲートウェアモードにいる場合、8080ポートを持つZVMODELVZにアクセスしてください（ブラウザにhttp://192.168.123.1:8080などのURLを入力することを意味します）。ブラウザ(80)の既定ポートは、ローカルネットワーク内でWebサーバー用に特別に予約されているため、このオプションはファイアウォールが有効になっている場合のみ機能します。ファイアウォールが無効になると、80ポートでZVMODELVZに直接アクセスできます。', LEFT);" onMouseOut="return nd();">WANからWebアクセスを有効にしますか?
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドにより、インターネットからWebサーバーにアクセスするために使用するポートを指定できます。', LEFT);" onMouseOut="return nd();">WANからWebアクセスのポート:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('この機能により、インターネットからLPR要求に応答できます。', LEFT);" onMouseOut="return nd();">WANからのLPR要求に応答しますか?
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('この機能により、インターネットからピング要求に応答できます。', LEFT);" onMouseOut="return nd();">WANからのピング要求に応答しますか?
