...
<td class="content_header_td_title" colspan="2">ワイヤレス - RADIUS設定</td>
</tr>
<tr>
<td class="content_desc_td" colspan="2">このセクションでは、RADIUSサーバーを通して、ワイヤレスクライアントに権限を与えるための追加パラメータを設定します。これは、「WPA」または「802.1xを持つRadius」として「ワイヤレス - インターフェイス」で「認証方式」を選択している間、要求されます。
         </td>
</tr>
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドはRADIUSサーバーのIPアドレスを指定して、802.1Xワイヤレス認証とダイナミックWEPキー導出を使用します。', LEFT);" onMouseOut="return nd();">サーバーIPアドレス:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、RADIUSサーバーで使用されるUDPポート番号を指定します。', LEFT);" onMouseOut="return nd();">サーバーポート:
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('このフィールドは、RADIUSサーバーで接続を初期化するために使用されるパスワードを指定します。', LEFT);" onMouseOut="return nd();">接続機密:
