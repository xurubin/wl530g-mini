...
<tr>
<td class="content_header_td" onMouseOver="return overlib('USBディスクを挿入後、ZVMODELVZはUSBディスクのルートディレクトリの指名された初期スクリプトを探し、それを実行します。これをブランクにすると、この機能が無効になります。', LEFT);" onMouseOut="return nd();">初期スクリプト:
