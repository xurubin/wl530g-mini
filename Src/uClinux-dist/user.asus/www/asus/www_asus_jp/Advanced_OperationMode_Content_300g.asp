...
ZVMODELVZは2つの操作モードをサポートして、異なるグループのユーザーから異なる要件を満たします。状況に一致するモードを選択してください。
</td>
</tr>
...
アクセスポイントモードで、イーサネットポートとワイヤレスデバイスは同じローカルエリアネットワークで配置するように設定されます。機能に関連するこれらのWANはここではサポートされていません。
<p></p>
専門用語の説明をすると、アクセスポイントモードは、NATが無効、ZVMODELVZのLANポートとワイヤレスポートが互いにブリッジされています。
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode"> ホームゲートウェイ</td>
<td class="content_desc_td">
このモードで、ユーザーはZVMODELVZのイーサネットポートを１つだけを使用し、ADSLまたはケーブルモデムを通してインターネットに接続するものと、また、お使いの環境にはIPとISPを共有する多くのユーザーがいるものと仮定します。
<p></p>
