...
<tr>
<td class="content_header_td" onMouseOver="return overlib('如果您启用了UPnP，操作系统，例如Windows XP，会自动发现ZVMODELVZ。同时，UPnP可让操作系统自动设置ZVMODELVZ状态以适用各种互联网应用程序，例如线上游戏及视讯会议。', LEFT);" onMouseOut="return nd();">启用UPnP？
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本项功能可让您指定远端服务器来记录与ZVMODELVZ有关的记录信息。如果不作设置，系统仅会在ZVMODELVZ上记载最多为1024道的信息。', LEFT);" onMouseOut="return nd();">远程日志服务器（Remote Log Server）：
...
<tr>
<td class="content_desc_td" colspan="2">动态DNS（DDNS）让您即使在没有静态IP地址的情况下，仍可将服务器连同特有名称一并汇出至互联网上。目前，在ZVMODELVZ之中已嵌入数台的DDNS用户端。您可点选下方的「免费试用」，来启用一个免费试用帐号。
         </td>
</tr>
<tr>
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('此键可让您以手动方式更新DDNS数据库。但仅在DDNS自动更新失败时才能使用。您可以自「系统日志」（System Log）中取得目前DDNS更新的状态。', LEFT);" onMouseOut="return nd();">手动更新：
