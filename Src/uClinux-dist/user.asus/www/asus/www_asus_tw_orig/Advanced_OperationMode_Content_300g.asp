...
ZVMODELVZ는 그룹 구성원들의 요구사항을 충족시키기 위하여 2가지의 작동모드를 지원합니다. 상황에 맞는 모드를 선택하십시오.
</td>
</tr>
...
액세스 포인트 모드에서는 이더넷 포트와 무선 디바이스는 동일한 로컬 영역 네트워크에 설정할 수 있습니다. WAN 관련 기능은 지원하지 않습니다.
<p></p>
전문적인 용어로 설명하면 액세스 포인트 모드에서는 NAT가 비활성화 되어 있고 ZVMODELVZ의 LAN 포트와 무선 포트가 서로 연결됩니다.
</td>
</tr>
<tr>
<td class="content_header_td_30"><input type="radio" name="x_Mode"> 홈 게이트웨이</td>
<td class="content_desc_td">
본 모드에서는 ADSL 또는 케이블 모뎀을 통한 인터넷 연결을 위하여 ZVMODELVZ 이더넷 포트만을 사용하고 있고 ISP와 동일한 IP 환경을 공유한 사람들이 많이 있는 것으로 가정합니다.
<p></p>
전문적인 용어로 설명하면 게이트웨이 모드에서는 NAT가 활성화 되고 PPPoE나 DHCP 클라이언트 또는 고정IP를 사용하여 WAN 연결이 됩니다. 그리고 UPnP와 DDNS와 같이 홈 사용자들에게 유용한 몇 가지의 특징들이 지원됩니다.
</td>
</tr>
<tr>
