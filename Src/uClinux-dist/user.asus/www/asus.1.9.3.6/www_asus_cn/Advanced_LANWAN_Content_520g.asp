...
<tr>
<td class="content_desc_td" colspan="2">ZVMODELVZ可支持数种连接WAN的连接类型。这些类型可从WAN连接类型旁的下拉式选单中选取。设置栏目会根据您是选取那种连接类型而定。
         </td>
</tr>
<tr>
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('选择「停用」，会将WAN端口停用并将ZVMODELVZ设成是一台AP。选择「启用」，会将ZVMODELVZ设成是一台含有NAT功能的路由器。在开机期间将以太网络线接上WAN端口时，选择「自动」可将ZVMODELVZ设成是一台路由器。', LEFT);" onMouseOut="return nd();">WAN模式：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('此ZVMODELVZ的IP地址与您在远程网络上所见相同。如果您空着不填，ZVMODELVZ会自动向DHCP服务器取得IP地址。', LEFT);" onMouseOut="return nd();">IP地址：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('此一ZVMODELVZ的IP地址与您在远程网络上所见相同。', LEFT);" onMouseOut="return nd();">子网掩码：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('此为默认网关的IP地址，可使ZVMODELVZ与远程网络或主机之间相互连系。', LEFT);" onMouseOut="return nd();">默认网关：
...
<tr>
<td class="content_header_td">自动连接至DNS服务器？
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('此一栏目可显示ZVMODELVZ所连接的DNS服务器的IP地址。', LEFT);" onMouseOut="return nd();">DNS服务器1：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('此一栏目可显示ZVMODELVZ所连接的DNS服务器的IP地址。', LEFT);" onMouseOut="return nd();">DNS服务器2：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目可让您设置在一特定时间间隔之后终止您ISP连接的状态。数值0可允许无限延长间置时间。如果仅勾选Tx，那么来自互联网的资料将会跳过以继续计算间置的时间。', LEFT);" onMouseOut="return nd();">闲置断线时间（可选）：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib(' PPPoE封包的最大传输单元（MTU）。', LEFT);" onMouseOut="return nd();">最大传输单元（MTU）：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib(' PPPoE封包的最大接收单元（MRU）。', LEFT);" onMouseOut="return nd();">最大接收单元（MRU）：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本项可由ISP指定。请向您的ISP确认并在需要时将填入。', LEFT);" onMouseOut="return nd();">访问集中器（Access Concentrator）名称（选项）：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('启用PPPoE Relay可让局域网（LAN）当中的工作站设置可通过网络地址转换（NAT）服务器的建立单独的PPPoE连接。', LEFT);" onMouseOut="return nd();">启用PPPoE Relay（PPPoE中继功能）？
...
<tr class="content_section_header_tr">
<td class="content_section_header_td" colspan="2">特定参数（需由ISP提供）
            </td>
</tr>
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目可让您提供ZVMODELVZ所使用的主机名称。通常是您ISP要求的名称。', LEFT);" onMouseOut="return nd();">主机名称：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目可让您提供ZVMODELVZ连接互联网所使用的特有MAC（媒体访问控制）地址。通常是您ISP要求的地址。', LEFT);" onMouseOut="return nd();"> MAC地址：
