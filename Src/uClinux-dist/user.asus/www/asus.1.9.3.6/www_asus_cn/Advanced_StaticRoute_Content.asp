...
<td class="content_header_td_title" colspan="2">路由器 - 静态路由</td>
</tr>
<tr>
<td class="content_header_td" onMouseOver="return overlib('将「静态路由表」（Static Route List）里所指定静态路由项目当中的路由信息，重新分配到RIP表内。', LEFT);" onMouseOut="return nd();">将静态路由重新分配到RIP（路由信息协议）内？
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('设置在传送通知（announcement）时路径适用的合适的顺序。针对RIP，有效的顺序是从1到16。', LEFT);" onMouseOut="return nd();">设置静态路由的度量标准（1-16）：
...
<tr>
<td class="content_header_td" onMouseOver="return overlib('本栏目仅针对「静态路由过滤器表」（Static Route Filter table）中所指定的合适目的网络，确定是否需要重新分配。', LEFT);" onMouseOut="return nd();">只有路由过滤器中指定的路由才需设置。
