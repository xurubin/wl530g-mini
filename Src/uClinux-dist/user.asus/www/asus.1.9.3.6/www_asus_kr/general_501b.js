...
{       
  ...
  if (num>65535 || (prev>num))
  {
     if (num>65535)
        ...
        alert(o.value + ' 유효하지 않은 포트번호!range');
     o.value = "";   
     o.focus();
     return false;
  } 
...
function change_common(o, s, v)
{
  ...
  {     	         	
      if(s == "WLANConfig11a")
      {
      	 ...
      	 {
      	     rst = confirm("변경하면 AES는 비활성화 됩니다!");
      	     if (rst)
      	     {
...
function changeWPA(wepStr, loadflag)
{
    ...
    else if (authType.value == "3") // WPA
    {
    	...
    	else
    	{
       	...
       	if (!loadflag) alert('이 옵션의 선택은 RADIUS 서버와의 연결을 요구합니다. "무선-RADIUS" 관련 파라미터의 설정을 기억하십시오.');
    }
