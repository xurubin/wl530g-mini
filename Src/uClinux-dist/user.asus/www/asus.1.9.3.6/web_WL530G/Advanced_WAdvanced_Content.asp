...
<tr>
<td class="content_header_td" onMouseOver="return overlib('스테이션을 선택하면 WL530G는 무선-인터페이스 페이지에서 정한 AP로 연결됩니다. 이더넷 브릿지를 선택하면 WL530G는 무선-인터페이스 페이지에서 정한 AP로 연결되고 WL530G의 LAN 포트에 연결된 디바이스들이 본 AP로도 연결됩니다. 스테이션과 이더넷 브릿지 모드는 오픈 시스템이나 공유키 또는 WPA-PSK로 설정된 인증방식에서만 작용합니다.', LEFT);" onMouseOut="return nd();">확장모드:
