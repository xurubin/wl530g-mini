 ...
 *
 * $Id: firewall.c,v 1.2 2006/02/10 06:10:16 Eric1_Liu Exp $
 */
...
start_firewall2(char *wan_ifname)
{
	...
	if (nvram_invmatch("http_wanport", "")) {
		...
		//eric++, for Terayon requirement 1.e 
		// (WX-54G was accessible with original IP address after L2TP tunnel had been created.
		/* Set up WAN side match */
		if (strcmp (nvram_safe_get ("wan_proto"), "l2tp") == 0 && nvram_invmatch ("wan_dhcp_ipaddr", ""))	{
			memset(&nat, 0, sizeof(nat));
			nat.match.ipproto = IPPROTO_TCP;
			nat.match.src.ports[1] = htons(0xffff);
			nat.match.dst.ports[0] = htons(atoi(nvram_safe_get("http_wanport")));
			...
			inet_aton("255.255.255.255", &nat.match.dst.netmask);
			inet_aton(nvram_safe_get ("wan_dhcp_ipaddr") , &nat.match.dst.ipaddr);
	...
	if (nvram_match(strcat_r(prefix, "proto", tmp), "pppoe")||nvram_match(strcat_r(prefix, "proto", tmp), "l2tp"))
