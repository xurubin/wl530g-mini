 ...
 *
 * $Id: udhcpc.c,v 1.2 2006/02/10 06:10:15 Eric1_Liu Exp $
 */
...
#ifdef WX54G	
	/* start up wan with static ip for snap ap */	
	ifconfig(wan_ifname, IFUP,
...
bound(void)
{
	...
	if ((value = getenv("ip")))	{
		...
		// eric++, pass WAN IP address to start_firewall2() (for Terayon requirement 1.e)
		if (strcmp (nvram_safe_get (strcat_r (prefix, "proto", tmp)), "l2tp") == 0)	{
			nvram_set ("wan_dhcp_ipaddr", value);
		}
	}
