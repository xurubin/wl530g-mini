/*
 * Firewall services, extension by iptables
 *
 ...
 *
 * $Id: firewall.c,v 1.34 2003/04/11 00:21:09 jqliu Exp $
 */
...
ip2class(char *lan_ip, char *netmask, char *buf)
{
	...
	if (val==0xff00000) sprintf(buf, "%s/8", lan_ip);
	else if (val==0xffff0000) sprintf(buf, "%s/16", lan_ip);
	else sprintf(buf, "%s/24", lan_ip);
	printf(buf);	
}
...
start_firewall_ex(char *wan_if, char *wan_ip, char *lan_if, char *lan_ip)
{
	...
	/* nat setting */
	logmessage("nat_setting", "wan_if %s wan_ip %s lan_if %s lan_ip %s", wan_if, wan_ip, lan_if, lan_ip); //eric++
...
get_forward_port_ex(int which, netconf_nat_t *nat)
{
	...
	char *enable, *desc;
	char *lan_ipaddr, *lan_port, *proto;
	int  in_start, in_end;
	char lanwan_timematch[128];
	char ptr[32], *icmplist;
