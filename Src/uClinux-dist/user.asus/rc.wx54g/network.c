 ...
 *
 * $Id: network.c,v 1.3 2006/02/07 09:23:42 Eric1_Liu Exp $
 */
...
/*		if (strcmp(wan_proto, "l2tp")==0) 
		...
		else if (nvram_match("wan_proto", "pppoe")) 
		{
			...
			char *pppoe_argv[] = { "/bin/pppd",
					"pty",
					pppoe_parameters,
					"user", nvram_safe_get("wan0_pppoe_username"),
					"demand", "connect", "true",
					"persist", 
					"usepeerdns",
			...
			fp = fopen("/tmp/ppp/pap-secrets", "w+");

			if (fp!=NULL)
			{
				fprintf(fp, "%s\t*\t%s\t", 
				nvram_safe_get("wan0_pppoe_username"), 
				nvram_safe_get("wan0_pppoe_passwd"));
				fclose(fp);				
			}

			fp = fopen("/tmp/ppp/chap-secrets", "w+");
...
/*JYWeng: 20040130*/
			...
			if (nvram_invmatch("wan0_pppoe_ac", ""))
				sprintf(pppoe_parameters, " -C %s", nvram_safe_get("wan0_pppoe_ac"));

			if (nvram_invmatch("wan0_pppoe_service", ""))
				sprintf(pppoe_parameters, " -S %s", nvram_safe_get("wan0_pppoe_service") );


			for (arg=pppoe_argv; *arg; arg++);

			if (nvram_invmatch("wan0_pppoe_idletime", ""))
			{
				*arg++ = "idle";
				*arg++ = nvram_safe_get("wan0_pppoe_idletime");
			}

			if (nvram_match("wan0_pppoe_txonly_x", "1"))
			{
				*arg++ = "tx_only";
			}
			if (nvram_invmatch("wan0_pppoe_mtu", "")) 
			{
				*arg++ = "mtu";
				*arg++ = nvram_safe_get("wan0_pppoe_mtu");
			}
			if (nvram_invmatch("wan0_pppoe_mru", "")) 
			{
				*arg++ = "mru";
				*arg++ = nvram_safe_get("wan0_pppoe_mru");
		...
		*/
			...
			char *dhcp_argv[] = { "udhcpc",
					      "-i", wan_ifname,				      				      "-p", "/var/run/udhcpc0.pid",
...
wan_up(char *wan_ifname)
{
	...
	if (nvram_match(strcat_r(prefix, "primary", tmp), "1"))	{
		// If 
		//	1. wan_proto is "l2tp"
		//	2. IP address of default gateway is equal to IP address of WAN interface, and
		// ==> do not add it as default route.

//		isl2tp = !strcmp (wan_proto, "l2tp");
//		p1 = nvram_safe_get (strcat_r (prefix, "ipaddr", tmp));
//		p2 = nvram_safe_get (strcat_r (prefix, "gateway", tmp));
//		if (!isl2tp || (isl2tp && strcmp (p1, p2)))	{
...
#ifdef ASUS_EXT
	if ((strcmp(wan_proto, "l2tp")==0) || (strstr(wan_ifname, "ppp")==0))
		update_wan_status(1);
	else
		update_wan_status(0);
...
wan_up_none(char *wan_ifname)
{
	...
	nvram_set("wan_ipaddr_t", nvram_safe_get("wan_ipaddr"));
	nvram_set("wan_netmask_t", nvram_safe_get("wan_netmask"));
	nvram_set("wan_gateway_t", nvram_safe_get("wan_gateway"));
	nvram_set("wan_dns_t", "");
...
wan_down(char *wan_ifname)
{
	...
	if ((strcmp(wan_proto, "l2tp")==0)&& strstr(wan_ifname, "ppp"))
	{
		stop_l2tp();
		if(nvram_match("l2tp_down", "0"))
		{
			nvram_set("l2tp_down","1");
		}
	}
#endif
