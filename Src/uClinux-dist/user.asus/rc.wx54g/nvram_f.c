...
{            
   return(nvram_set_f(sid, name, value));
}
...
{	
   ...
   value  = strtrim_c(nvram_get_f(sid, name));
...
{
    FILE *fl;           
    char const buf[MAX_LINE_SIZE];	
    char filename[MAX_FILE_NAME];
    unsigned char *v, *sp, *tail;
       ...
       if (v != NULL && ((sp = strchr(buf, ' ')) == NULL || (sp > v))) {
            ...
            *v++ = '\0';           
            ...
            {  
               ...
               nvram_set_item(&curEnv, buf, strtrim(v));
               break;               
            }   
       }     
    }  
...
nvram_set_f(const char *file, const char *field, const char *value)
{
    ...
    {    	    
       ...
       if (v != NULL && ((sp = strchr(buf, ' ')) == NULL || (sp > v))) {
            /* change the "name=val" string to "set name val" */                                    
            if (strncmp(buf, field, strlen(field))==0 && buf[strlen(field)]=='=')
            {  
               if (value!=NULL)	
               {
                  sprintf(v+1, "\"%s\"", value);	                          	                    
