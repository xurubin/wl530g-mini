...
void usl_fd_poll(void)
{
...
#ifdef WX54G
		if (break_usl_fd_pool)	{
			l2tp_log (LOG_INFO, "break_usl_fd_pool is true, close %s()\n", __FUNCTION__);
			break;
		}
#endif
	...
	if (usl_fd_poll_hook != NULL) {
		//eric, (*usl_fd_poll_hook) is reserved for apps. openl2tpd didn't use it.
		(*usl_fd_poll_hook)();
	}
