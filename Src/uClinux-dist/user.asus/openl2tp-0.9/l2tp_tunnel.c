...
struct l2tp_tunnel {
	...
	l2tp_avp_challenge_t			*my_challenge;
	int						my_challenge_len;
	l2tp_avp_result_code_t			*result_code;		/* we send this info to peer when closing tunnels */
	int						result_code_len;
};
...
{
	unsigned hash = usl_hash_full_name((const unsigned char*)tunnel_name, strlen(tunnel_name));
...
static l2tp_avp_result_code_t *l2tp_tunnel_emergency_result_code;
 ...
 */
static const l2tp_result_codes_t l2tp_tunnel_stopccn_result_codes[] = {
...
void l2tp_tunnel_set_addresses(struct l2tp_tunnel *tunnel, struct sockaddr_in *src, struct sockaddr_in *dest)
{
	...
	if (tunnel->config.do_pmtu_discovery) {
		...
		if (result == 0) {
			result = getsockopt(tunnel->fd, SOL_IP, IP_MTU, &mtu, (socklen_t*) &optlen);
			if (result == 0) {
...
static void l2tp_tunnel_log_error(struct l2tp_tunnel *tunnel,
				  l2tp_avp_result_code_t *result_code)
{
	const l2tp_result_codes_t *entry = &l2tp_tunnel_stopccn_result_codes[0];
				   ...
				   uint16_t error_code, char *error_string)
{
	...
	if (len > 4) {
		strncpy(&tunnel->result_code->error_message[0], error_string, len - sizeof (l2tp_avp_result_code_t));
	} else {
...
static int l2tp_tunnel_build_result_code(struct l2tp_tunnel *tunnel)
{
	 ...
	 */
	if (tunnel->result_code == NULL) {
		tunnel->result_code = malloc(sizeof(l2tp_avp_result_code_t));
		if (tunnel->result_code == NULL) {
			tunnel->result_code = l2tp_tunnel_emergency_result_code;
		}
...
static void l2tp_tunnel_act_send_sccrq(void *tun, void *session, void *arg3)
{
	...
	framing_cap.u.value = 0;
	framing_cap.u.bits.async = tunnel->config.framing_cap_async;
	framing_cap.u.bits.sync = tunnel->config.framing_cap_sync;
	...
	bearer_cap.u.value = 0;
	bearer_cap.u.bits.analog = tunnel->config.bearer_cap_analog;
	bearer_cap.u.bits.digital = tunnel->config.bearer_cap_digital;
...
static void l2tp_tunnel_act_send_sccrp(void *tun, void *sess, void *arg3)
{
	...
	l2tp_avp_tunnel_id_t tunnel_id;
	l2tp_avp_rx_window_size_t rx_window_size;
	l2tp_avp_challenge_t *challenge = NULL;
	l2tp_avp_challenge_response_t *challenge_response = NULL;
	struct l2tp_packet *pkt = NULL;
	l2tp_avp_framing_cap_t framing_cap;
	l2tp_avp_bearer_cap_t bearer_cap;
	int result;
...
static int l2tp_tunnel_store_avps(struct l2tp_tunnel *tunnel, l2tp_avp_desc_t *avps)
{
	int avp_len;
	...
	if (avps[L2TP_AVP_TYPE_FRAMING_CAP].value != NULL) {
		tunnel->peer.framing_cap.u.value = avps[L2TP_AVP_TYPE_FRAMING_CAP].value->framing_cap.u.value;
	}
	if (avps[L2TP_AVP_TYPE_BEARER_CAP].value != NULL) {
		tunnel->peer.bearer_cap.u.value = avps[L2TP_AVP_TYPE_BEARER_CAP].value->bearer_cap.u.value;
	}
		      ...
		      uint16_t session_id, struct l2tp_packet *pkt)
{
	uint16_t msg_type;
	int result;
	l2tp_avp_desc_t avp_desc[L2TP_AVP_TYPE_NUM_AVPS];
...
bool_t l2tp_tunnel_get_1_svc(u_short tunnel_id, optstring tunnel_name, struct l2tp_api_tunnel_msg_data *result, struct svc_req *req)
{
	...
	result->peer.framing_cap_sync = tunnel->peer.framing_cap.u.bits.sync;
	result->peer.framing_cap_async = tunnel->peer.framing_cap.u.bits.async;
	result->peer.bearer_cap_digital = tunnel->peer.bearer_cap.u.bits.digital;
	result->peer.bearer_cap_analog = tunnel->peer.bearer_cap.u.bits.analog;
...
void l2tp_tunnel_init(void)
{
	...
	l2tp_tunnel_emergency_result_code = malloc(sizeof(l2tp_avp_result_code_t) + 
	...
	memset(l2tp_tunnel_emergency_result_code, 0, 
	       sizeof(l2tp_avp_result_code_t) + L2TP_API_TUNNEL_EMERG_RESULT_CODE_SIZE);
...
//eric++
char* 
l2tp_tunnel_get_state_name (uint16_t tunl_id)
{
	char *retval = "";
	struct l2tp_tunnel *tunnel;
	...
	if (tunnel != NULL)	{
		retval = usl_fsm_state_name (&tunnel->fsmi);
	}
...
int
l2tp_tunnel_get_num_sessions (uint16_t tunl_id)
{
	int retval = 0;
	struct l2tp_tunnel *tunnel;
	...
	tunnel = l2tp_tunnel_find_by_id (tunl_id);
	if (tunnel != NULL)	{
		retval = tunnel->status.num_sessions;
	}
...
int
l2tp_tunnel_get_num_tunnels (void)
{
	return l2tp_tunnel_count;
}
