...
//eric--	if(getsockname(pppol2tp_fd, (struct sockaddr *)&s, &len) < 0) {
	 ...
	 * feature available. 
	 */
//eric--	if (getsockopt(pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_DEBUG, &tmp, &tmp_len) < 0) {
	if (getsockopt(pppol2tp_fd, SOL_PPPOL2TP, PPPOL2TP_SO_DEBUG, &tmp, (socklen_t*) &tmp_len) < 0) {
...
#if defined(WX54G) || defined (PLUGIN_L2TP)
void l2tp_plugin_init(void)
#else
