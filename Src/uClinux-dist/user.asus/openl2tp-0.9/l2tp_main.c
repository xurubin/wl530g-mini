/*****************************************************************************
 * Copyright (C) 2004,2005 Katalix Systems Ltd
 * 
...
int main(int argc, char **argv)
{
	...
	l2tp_firmware_revision = (((L2TP_APP_MAJOR_VERSION & 0x0f) << 4) | ((L2TP_APP_MINOR_VERSION & 0x0f) << 0));

	/* Become a daemon */
#ifdef WX54G
	if (!execed && !l2tp_opt_nodaemon) {
		unlink (L2TP_PID_FILENAME);
		usl_daemonize(argv[0]);
	}
#else
...
#ifdef WX54G
	// only child process execute following codes
	if (execed)	{
		setsid ();
		chdir ("/");
		umask (0);
	...
	write(fd, &pidstr[0], strlen(pidstr));
	close(fd);
...
//	l2tp_log(LOG_INFO, "Start, trace_flags=%08x%s", l2tp_opt_trace_flags, l2tp_opt_debug ? " (debug enabled)" : "");
...
static int l2tp_parse_args(int argc, char **argv)
{
	...
	while((opt = getopt(argc, argv, "p:d:u:L:fRDhc")) != -1) {
		switch(opt) {
		case 'h':
			usage(argv, 0);
			break;
...
{
#ifdef WX54G
	// WX54G may be blocked in usl_fd_read (l2tp_rand_fd)
	// I use rand() to replace it.
	
	unsigned char *p = buf;
	int i;

	srand (time (NULL) + (unsigned int) buf);
