...
static l2tp_avp_result_code_t *l2tp_session_emergency_result_code;
 ...
 * See RFC2661.
 */
static const l2tp_result_codes_t l2tp_session_cdn_result_codes[] = {
...
static void l2tp_session_log_error(struct l2tp_session *session,
				   l2tp_avp_result_code_t *result_code)
{
	const l2tp_result_codes_t *entry = &l2tp_session_cdn_result_codes[0];
				    ...
				    uint16_t error_code, char *error_string)
{
	...
	if (len > 4) {
		strncpy(&session->result_code->error_message[0], error_string, len - sizeof (l2tp_avp_result_code_t));
	} else {
...
static int l2tp_session_store_avps(struct l2tp_session *session, struct l2tp_tunnel *tunnel, 
				   l2tp_avp_desc_t *avps)
{
	int result = 0;
	int avp_len;

	if (avps[L2TP_AVP_TYPE_BEARER_TYPE].value != NULL) {
		session->peer.bearer_type.u.value = avps[L2TP_AVP_TYPE_BEARER_TYPE].value->bearer_type.u.value;
	}
	if (avps[L2TP_AVP_TYPE_FRAMING_TYPE].value != NULL) {
		session->peer.framing_type.u.value = avps[L2TP_AVP_TYPE_FRAMING_TYPE].value->framing_type.u.value;
	}
	...
	if (avps[L2TP_AVP_TYPE_CALL_ERRORS].value != NULL) {
		...
		if (avp_len < sizeof(l2tp_avp_call_errors_t)) {
			goto out_badmsg;
		}
		session->peer.call_errors = calloc(1, sizeof(l2tp_avp_call_errors_t));
		if (session->peer.call_errors == NULL) {
			l2tp_stats.no_avp_resources++;
			goto out_nomem;
		}
	...
	if (avps[L2TP_AVP_TYPE_ACCM].value != NULL) {
		...
		if (avp_len < sizeof(l2tp_avp_accm_t)) {
			goto out_badmsg;
		}
		session->peer.accm = calloc(1, sizeof(l2tp_avp_accm_t));
		if (session->peer.accm == NULL) {
			l2tp_stats.no_avp_resources++;
			goto out_nomem;
		}
...
static void l2tp_session_handle_msg_ocrq(struct l2tp_peer *peer, struct l2tp_tunnel *tunnel,
					 uint16_t session_id, l2tp_avp_desc_t *avps)
{
	int result;
...
static int l2tp_session_send_icrq(void *tun, void *sess)
{
	...
	l2tp_avp_desc_t avps[L2TP_AVP_TYPE_NUM_AVPS];
	l2tp_avp_message_type_t msg_type;
	...
	bearer_type.u.value = 0;
	bearer_type.u.bits.digital = session->config.bearer_type_digital;
	bearer_type.u.bits.analog = session->config.bearer_type_analog;
...
static int l2tp_session_send_iccn(void *tun, void *sess)
{
	...
	framing_type.u.value = 0;
	framing_type.u.bits.async = session->config.framing_type_async;
	framing_type.u.bits.sync = session->config.framing_type_sync;
	...
	if (session->config.proxy_auth_challenge != NULL) {
		...
		avps[L2TP_AVP_TYPE_PROXY_AUTH_CHALLENGE].value_len = strlen((char*) session->config.proxy_auth_challenge);
	}
	...
	if (session->config.proxy_auth_response != NULL) {
		...
		avps[L2TP_AVP_TYPE_PROXY_AUTH_RESPONSE].value_len = strlen((char*) session->config.proxy_auth_response);
	}
 ...
 */
//eric--bool_t l2tp_session_incall_ind_1_svc(int ppp_unit, struct l2tp_api_session_msg_data msg, char *tunnel_profile_name, int *result, struct svc_req *req)
bool_t l2tp_session_incall_ind_1_svc(int ppp_unit, struct l2tp_api_session_msg_data msg, int *result, struct svc_req *req)
{
	*result = -EOPNOTSUPP;
	return FALSE;
}
...
int l2tp_session_info_get(struct l2tp_session const *session, uint16_t tunnel_id, uint16_t session_id, struct l2tp_api_session_msg_data *result)
{
	...
	result->peer.framing_type_sync = session->peer.framing_type.u.bits.sync;
	result->peer.framing_type_async = session->peer.framing_type.u.bits.async;
	result->peer.bearer_type_digital = session->peer.bearer_type.u.bits.digital;
	result->peer.bearer_type_analog = session->peer.bearer_type.u.bits.analog;
...
void l2tp_session_init(void)
{
	...
	l2tp_session_emergency_result_code = malloc(sizeof (l2tp_avp_result_code_t) + L2TP_SES_EMERG_RESULT_CODE_SIZE);
	if (l2tp_session_emergency_result_code == NULL) {
		fprintf(stderr, "Out of memory\n");
		exit(1);
	}
	memset(l2tp_session_emergency_result_code, 0, sizeof (l2tp_avp_result_code_t) + L2TP_SES_EMERG_RESULT_CODE_SIZE);
...
//eric++
char*
l2tp_session_get_state_name (uint16_t tunl_id, uint16_t sess_id)
{
	struct l2tp_tunnel *tunnel;
	struct l2tp_session *session;
	char *retval = "";

	if (tunl_id == 0 || sess_id == 0)	{
		return retval;
	}

	tunnel = l2tp_tunnel_find_by_id (tunl_id);
	if (tunnel == NULL)	{
		return retval;
	}

	session = l2tp_session_find_by_id (tunnel, sess_id);
	if (session == NULL)	{
		return retval;
	}

	retval = usl_fsm_state_name (&session->fsmi);
