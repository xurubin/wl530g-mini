...
struct l2tp_result_codes_s {
	int result_code;
	int error_code;
	const char *error_string;
};
typedef struct l2tp_result_codes_s l2tp_result_codes_t;
...
//eric++
#if 0
#define dbg(f,args...)	l2tp_log (LOG_INFO, f, ##args)
#define DBG				l2tp_log (LOG_INFO, "%s %s() line: %d", __FILE__, __FUNCTION__, __LINE__)
#define ENTER			l2tp_log (LOG_INFO, ">> %s %s()", __FILE__, __FUNCTION__)
#define LEAVE			l2tp_log (LOG_INFO, "<< %s %s() line: %d", __FILE__, __FUNCTION__, __LINE__)
#else
...
extern int l2tp_avp_message_encode(uint16_t msg_type, struct l2tp_packet **ppkt, int hide, l2tp_avp_desc_t *avp_data, struct l2tp_tunnel *tunnel);
extern int l2tp_avp_message_decode(int msg_len, l2tp_avp_hdr_t *first_avp, l2tp_avp_desc_t *data, struct l2tp_tunnel const *tunnel);
extern int l2tp_avp_hidable(l2tp_avp_hdr_t *avp);
extern int l2tp_message_parse(void *msg, int msg_len, l2tp_avp_desc_t *parse_data, void **avp_buf, struct l2tp_tunnel const *tunnel);
extern int l2tp_avp_preparse(l2tp_avp_hdr_t *first_avp, int avp_data_len, char **host_name, uint16_t *msg_type);
...
#else
#define l2tp_tunnel_log(lt,t,c,l,f...)	do {} while (0)
#endif
...
//eric++
extern char* l2tp_tunnel_get_state_name (uint16_t tunl_id);
extern int l2tp_tunnel_get_num_sessions (uint16_t tunl_id);
extern int l2tp_tunnel_get_num_tunnels (void);
...
#else
#define l2tp_session_log(ls,s,c,l,f...)	do {} while (0)
#endif
...
//eric++
extern char* l2tp_session_get_state_name (uint16_t tunl_id, uint16_t sess_id);
