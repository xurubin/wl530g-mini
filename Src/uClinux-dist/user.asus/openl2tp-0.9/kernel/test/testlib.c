...
int ConnectSock( int sock, int serversock, int port, int t1, int s1, int t2, int s2 )
{
  ...
  sax.pppol2tp.fd = serversock;
  sax.pppol2tp.addr.sin_addr.s_addr = htonl( 0x7F000001 );
  sax.pppol2tp.addr.sin_port        = htons( port );
  sax.pppol2tp.addr.sin_family      = AF_INET;
  sax.pppol2tp.s_tunnel  = t1;
  sax.pppol2tp.s_session = s1;
  sax.pppol2tp.d_tunnel  = t2;
  sax.pppol2tp.d_session = s2;
