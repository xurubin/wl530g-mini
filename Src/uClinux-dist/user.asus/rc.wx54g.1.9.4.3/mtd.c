...
mtd_write(const char *path, const char *mtd)
{
	...
	for (erase_info.start = 0; erase_info.start < trx.len; erase_info.start += count) {
		...
		printf("Len: %x %x %x\n", len, erase_info.length, erase_info.start);
		...
		if (count==0)
		{
			...
			count = off = rlen = sizeof(struct trx_header);

			if (write(mtd_fd, &trx, rlen) != rlen) 
			{
				perror(mtd);
				goto fail;
			}
		...
		while((rlen=safe_fread(buf, 1, MIN(MAX_BUF_SIZE, len - off), fp))!=NULL)
		{		
			/* Update CRC */
			crc = crc32(&buf[0], rlen, crc);
