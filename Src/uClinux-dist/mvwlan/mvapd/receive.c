...
#ifdef TURBO_SETUP
	if (memcmp(pos, auto_link_marvell_hdr, sizeof(auto_link_marvell_hdr)) == 0) {

		AUTO_LINK_ReceiveTSFrame(hapd, sa, buf, len);
		return;
	}
#endif
...
static void handle_frame(hostapd *hapd, char *buf, size_t len)
{
	...
	if (ver == 3) {
		...
		if (memcmp(buf+sizeof(*hdr), "MICERR", 6) == 0) {
...
int hostapd_init_sockets(hostapd *hapd)
{
	...
	if (memcmp(hapd->own_addr, ifr.ifr_hwaddr.sa_data, ETH_ALEN)) {
		printf("Invalid HW-addr address 0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:"
