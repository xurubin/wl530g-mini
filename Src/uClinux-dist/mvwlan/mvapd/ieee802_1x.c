...
void ieee802_1x_send(hostapd *hapd, struct sta_info *sta, u8 type,
				   ...
				   int authorized)
{
	...
	param.cmd = MVWLAN_HOSTAPD_SET_FLAGS_STA;
...
void ieee802_1x_tx_key(hostapd *hapd, struct sta_info *sta, u8 id)
{
	...
	if (hapd->conf->wep_key_len > 0) {
		u8 *ikey;
		ikey = malloc(hapd->conf->wep_key_len);
		if (ikey == NULL ||
...
void ieee802_1x_receive(hostapd *hapd, u8 *sa, u8 *buf, size_t len)
{
	...
	case IEEE802_1X_TYPE_EAPOL_KEY:
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
				HOSTAPD_LEVEL_DEBUG, "received EAPOL-Key "
				"from STA");
		sta->eapol_sm->key_mgmt.eapol_key_ptr = (u8 *) hdr;
		key_mgmt_dump_eapol_key_frame(hapd, (EAPOL_KeyMsg_t *) (hdr+1));
		rx_eapol_ptr = (EAPOL_KeyMsg_t *)(hdr + 1);

		if (rx_eapol_ptr->k.key_info16 & KEY_INFO_ERROR) {
			key_mgmt_MIC_counter_measure_invoke(hapd);
			return;
		}

		if ((ntohl(rx_eapol_ptr->replay_cnt[1]) != sta->key_mgmt_info.counter) &&
			rx_eapol_ptr->k.key_info.key_type == 1)
			return;

		sta->eapol_sm->key_mgmt.msg_responsed = TRUE;
		eapol_sm_step(sta->eapol_sm);
		sta->eapol_sm->key_mgmt.msg_responsed = FALSE;
		break;
...
{
#ifndef AP_WPA2
	if (!hapd->conf->ieee802_1x && !hapd->conf->wpa_mode)
#else
	if (!hapd->conf->ieee802_1x && !hapd->conf->wpa_mode && !hapd->conf->wpa2_mode)
#endif
		return;
...
static int ieee802_1x_init_kernel_1x(hostapd *hapd)
{
	...
	if (hostap_ioctl_mvWLANparam(hapd, MVWLAN_PARAM_IEEE_802_1X, 1)) {
	...
	if (hostap_ioctl_mvWLANparam(hapd, MVWLAN_PARAM_HOST_DECRYPT, 1) ||
	    hostap_ioctl_mvWLANparam(hapd, MVWLAN_PARAM_HOST_ENCRYPT, 1)) {
