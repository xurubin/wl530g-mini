...
struct eapol_key_mgmt_sm {

	unsigned int msg_to_time;
#define KEY_MGMT_MSG_TIMEOUT         2    /* 2 s */
	unsigned int msg_send_cnt;
	unsigned int msg_send_max;
#define KEY_MGMT_MSG_SEND_MAX        2    /* two times */
	unsigned int grpmsg_send_max;
#define KEY_MGMT_GRPMSG_SEND_MAX  5    /* five times */
 	u8 *eapol_key_ptr; /* point to 802.1x header of EAPOL key message */
	Boolean msg_responsed;
	Boolean grp_key_timeout;
	Boolean rekeyed;

	enum { KEY_MGMT_INITIALIZE, KEY_MGMT_SEND_MSG1,
		KEY_MGMT_SEND_MSG3, KEY_MGMT_SEND_GRPMSG1,
		KEY_MGMT_TIMEOUT, KEY_MGMT_END
	} state;
};
...
struct eapol_state_machine {
	...
	struct eapol_key_mgmt_sm key_mgmt;
