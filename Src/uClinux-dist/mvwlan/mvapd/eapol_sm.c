...
#define txKeyMgmtMsg1 key_mgmt_send_msg1(sm->hapd, sm->sta)
#define txKeyMgmtMsg3 key_mgmt_send_msg3(sm->hapd, sm->sta)
#define txKeyMgmtGrpMsg1 key_mgmt_send_grp_msg1(sm->hapd, sm->sta)
#define rxKeyMgmtMsg2 key_mgmt_recv_msg2(sm->hapd, sm->sta)
#define rxKeyMgmtMsg4 key_mgmt_recv_msg4(sm->hapd, sm->sta)
#define rxKeyMgmtGrpMsg2 key_mgmt_recv_grp_msg2(sm->hapd, sm->sta)
...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
	...
	eloop_register_timeout(0, 100000, eapol_port_timers_tick, eloop_ctx, state);
}
...
{
#ifndef AP_WPA2
	if (sm->hapd->conf->wpa_mode)
#else
	if ((sm->hapd->conf->wpa_mode) || (sm->hapd->conf->wpa2_mode))
#endif
		return;
...
/* Authenticator key management state machine */

SM_STATE(KEY_MGMT, INITIALIZE)
{
	SM_ENTRY(KEY_MGMT, INITIALIZE, key_mgmt);

	sm->key_mgmt.msg_to_time = KEY_MGMT_MSG_TIMEOUT;
	sm->key_mgmt.msg_send_cnt = 0;
	sm->key_mgmt.msg_send_max = KEY_MGMT_MSG_SEND_MAX;
	sm->key_mgmt.grpmsg_send_max = KEY_MGMT_GRPMSG_SEND_MAX;
	sm->key_mgmt.msg_responsed = FALSE;
	sm->key_mgmt.grp_key_timeout = FALSE;
	sm->key_mgmt.rekeyed = FALSE;
}


SM_STATE(KEY_MGMT, SEND_MSG1)
{
	SM_ENTRY(KEY_MGMT, SEND_MSG1, key_mgmt);

	sm->aWhile = sm->key_mgmt.msg_to_time;
	sm->key_mgmt.msg_responsed = FALSE;
	sm->key_mgmt.msg_send_cnt++;
	txKeyMgmtMsg1;
}


SM_STATE(KEY_MGMT, SEND_MSG3)
{
	SM_ENTRY(KEY_MGMT, SEND_MSG3, key_mgmt);
...
SM_STATE(KEY_MGMT, SEND_GRPMSG1)
{
	SM_ENTRY(KEY_MGMT, SEND_GRPMSG1, key_mgmt);
...
SM_STATE(KEY_MGMT, TIMEOUT)
{
	SM_ENTRY(KEY_MGMT, TIMEOUT, key_mgmt);

	/* deauthenticate this station */
	ieee802_11_send_deauth(sm->hapd, sm->sta->addr, WLAN_REASON_IE_IN_4WAY_DIFFERS);

	/* clean up station management information */
	sm->sta->flags &= ~(WLAN_STA_AUTH | WLAN_STA_ASSOC);
	remove_sta(sm->hapd, sm->sta);
}



SM_STATE(KEY_MGMT, END)
{
	SM_ENTRY(KEY_MGMT, END, key_mgmt);
}
...
SM_STEP(KEY_MGMT)
{
	if (sm->initialize) {
		sm->portStatus = Unauthorized;
		setPortUnauthorized();
		SM_ENTER(KEY_MGMT, INITIALIZE);
		return;
	}

	switch (sm->key_mgmt.state) {
	case KEY_MGMT_INITIALIZE:
		sm->key_mgmt.msg_send_cnt = 0;
		SM_ENTER(KEY_MGMT, SEND_MSG1);
		break;

	case KEY_MGMT_SEND_MSG1:
		if (sm->aWhile == 0) {

			if (sm->key_mgmt.msg_send_cnt >= sm->key_mgmt.msg_send_max) {
				SM_ENTER(KEY_MGMT, TIMEOUT);
			} else {
				SM_ENTER(KEY_MGMT, SEND_MSG1);
			}
		} else if (sm->key_mgmt.msg_responsed && rxKeyMgmtMsg2) {

			sm->key_mgmt.msg_send_cnt = 0;
			SM_ENTER(KEY_MGMT, SEND_MSG3);
		} else if (sm->auth_pae.eapStart == TRUE) {

			SM_ENTER(KEY_MGMT, SEND_MSG1);
		}
		break;

	case KEY_MGMT_SEND_MSG3:
		if (sm->aWhile == 0) {
		...
		} else if (sm->key_mgmt.msg_responsed && rxKeyMgmtMsg4) {

			sm->key_mgmt.msg_send_cnt = 0;
			sm->portStatus = Authorized;
			setPortAuthorized();
			SM_ENTER(KEY_MGMT, SEND_GRPMSG1);
		}
		break;

	case KEY_MGMT_SEND_GRPMSG1:
		if (sm->aWhile == 0) {

			if (sm->key_mgmt.msg_send_cnt >= sm->key_mgmt.grpmsg_send_max) {
				SM_ENTER(KEY_MGMT, TIMEOUT);
			} else {
				SM_ENTER(KEY_MGMT, SEND_GRPMSG1);
			}
		} else if (sm->key_mgmt.msg_responsed && rxKeyMgmtGrpMsg2) {

				SM_ENTER(KEY_MGMT, END);
		}
		break;

	case KEY_MGMT_TIMEOUT:
		sm->portStatus = Unauthorized;
		setPortUnauthorized();
		SM_ENTER(KEY_MGMT, END);
		break;
	...
	case KEY_MGMT_END:
		if (sm->key_mgmt.grp_key_timeout) {
			sm->key_mgmt.grp_key_timeout = FALSE;
			sm->key_mgmt.msg_send_cnt = 0;
			SM_ENTER(KEY_MGMT, SEND_GRPMSG1);
		} else if ( sm->key_mgmt.rekeyed) {
			sm->key_mgmt.rekeyed = FALSE;
			sm->key_mgmt.msg_send_cnt = 0;
			SM_ENTER(KEY_MGMT, SEND_MSG1);
		}
...
{
	int prev_auth_pae, prev_be_auth, prev_reauth_timer, prev_auth_key_tx, prev_key_mgmt;
	...
	do {
		...
		prev_key_mgmt = sm->key_mgmt.state;

		SM_STEP_RUN(AUTH_PAE);
		SM_STEP_RUN(BE_AUTH);
		SM_STEP_RUN(REAUTH_TIMER);
		SM_STEP_RUN(AUTH_KEY_TX);
		SM_STEP_RUN(KEY_MGMT);
...
void eapol_sm_initialize(struct eapol_state_machine *sm)
{
	...
	eloop_cancel_timeout(eapol_port_timers_tick, sm->hapd, sm);
	eloop_register_timeout(0, 100000, eapol_port_timers_tick, sm->hapd, sm);
}
