...
{
	if (val) 
		gpioClrRegBits(GPIO_REG_OUTPUTS, (1<<port)); // GPIO on
	else 
		gpioSetRegBits(GPIO_REG_OUTPUTS, (1<<port));                // LED off
...
// Dev nodes are created in the filesystem manually.
	if(devfs_register_chrdev(gpio_major, "gpio", &gpio_fops) < 0)
		return -1;
	for (i = 0; i < ARRAYSIZE(gpio_file); i++) {
		gpio_file[i].handle = devfs_register(NULL,
						     gpio_file[i].name,
