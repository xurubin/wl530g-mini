...
GT_STATUS qdStart(void) /* devId is used for simulator only */
{
	...
	*/
	...
	if((status = gvlnSetPortVid(qd_dev, GT_CPU_SWITCH_PORT, 1)) != GT_OK) {
...
void pReadPortReg(int port, int reg)
{
	GT_U16 			tmpData;

	hwReadPortReg(qd_dev, (GT_U8)port, reg, &tmpData);

	gtOsPrintf("Port %d, Reg %d -> 0x%X\n", port, reg, tmpData);
}

void pWritePortReg(int port, int reg, GT_U16 data)
{
	GT_U16	tmpData;

	hwWritePortReg(qd_dev, (GT_U8)port, reg, data);
